 ...
 * Incept:    ML, Fri Mar 12 11:03:02 2021 [Heidelberg]
 *
...
p7_oprofile_Create(int allocM, const ESL_ALPHABET *abc)
{
  ...
  /* level 1 */
  ESL_ALLOC(om->rbv_mem, sizeof(uint8x16_t)  * nqb  * abc->Kp          +15); /* +15 is for manual 16-byte alignment */
  ESL_ALLOC(om->sbv_mem, sizeof(uint8x16_t)  * nqs  * abc->Kp          +15);
  ESL_ALLOC(om->rwv_mem, sizeof(int16x8_t)   * nqw  * abc->Kp          +15);
  ESL_ALLOC(om->twv_mem, sizeof(int16x8_t)   * nqw  * p7O_NTRANS       +15);
  ESL_ALLOC(om->rfv_mem, sizeof(float32x4_t) * nqf  * abc->Kp          +15);
  ESL_ALLOC(om->tfv_mem, sizeof(float32x4_t) * nqf  * p7O_NTRANS       +15);

  ESL_ALLOC(om->rbv, sizeof(uint8x16_t  *) * abc->Kp);
  ESL_ALLOC(om->sbv, sizeof(uint8x16_t  *) * abc->Kp);
  ESL_ALLOC(om->rwv, sizeof(int16x8_t   *) * abc->Kp);
  ESL_ALLOC(om->rfv, sizeof(float32x4_t *) * abc->Kp);

  /* align vector memory on 16-byte boundaries */
  om->rbv[0] = (uint8x16_t  *) (((unsigned long int) om->rbv_mem + 15) & (~0xf));
  om->sbv[0] = (uint8x16_t  *) (((unsigned long int) om->sbv_mem + 15) & (~0xf));
  om->rwv[0] = (int16x8_t   *) (((unsigned long int) om->rwv_mem + 15) & (~0xf));
  om->twv    = (int16x8_t   *) (((unsigned long int) om->twv_mem + 15) & (~0xf));
  om->rfv[0] = (float32x4_t *) (((unsigned long int) om->rfv_mem + 15) & (~0xf));
  om->tfv    = (float32x4_t *) (((unsigned long int) om->tfv_mem + 15) & (~0xf));
 ...
 * Incept:    ML, Fri Mar 12 11:08:30 2021 [Heidelberg]
 *
...
p7_oprofile_Sizeof(P7_OPROFILE *om)
{
  ...
  n  += sizeof(uint8x16_t)  * nqb  * om->abc->Kp +15; /* om->rbv_mem   */
  n  += sizeof(uint8x16_t)  * nqs  * om->abc->Kp +15; /* om->sbv_mem   */
  n  += sizeof(int16x8_t)   * nqw  * om->abc->Kp +15; /* om->rwv_mem   */
  n  += sizeof(int16x8_t)   * nqw  * p7O_NTRANS  +15; /* om->twv_mem   */
  n  += sizeof(float32x4_t) * nqf  * om->abc->Kp +15; /* om->rfv_mem   */
  n  += sizeof(float32x4_t) * nqf  * p7O_NTRANS  +15; /* om->tfv_mem   */

  n  += sizeof(uint8x16_t  *) * om->abc->Kp;          /* om->rbv       */
  n  += sizeof(uint8x16_t  *) * om->abc->Kp;          /* om->sbv       */
  n  += sizeof(int16x8_t   *) * om->abc->Kp;          /* om->rwv       */
  n  += sizeof(float32x4_t *) * om->abc->Kp;          /* om->rfv       */
 ...
 * Synopsis:  Allocate an optimized profile structure.
 * Incept:    ML, Fri Mar 12 11:13:38 2021 [Heidelberg]
 *
 * Purpose:   Allocate for profiles of up to <allocM> nodes for digital alphabet <abc>.
 *
 * Throws:    <NULL> on allocation error.
...
p7_oprofile_Copy(P7_OPROFILE *om1)
{
  ...
  /* level 1 */
  ESL_ALLOC(om2->rbv_mem, sizeof(uint8x16_t)  * nqb  * abc->Kp    +15);	/* +15 is for manual 16-byte alignment */
  ESL_ALLOC(om2->sbv_mem, sizeof(uint8x16_t)  * nqs  * abc->Kp    +15);
  ESL_ALLOC(om2->rwv_mem, sizeof(int16x8_t)   * nqw  * abc->Kp    +15);
  ESL_ALLOC(om2->twv_mem, sizeof(int16x8_t)   * nqw  * p7O_NTRANS +15);
  ESL_ALLOC(om2->rfv_mem, sizeof(float32x4_t) * nqf  * abc->Kp    +15);
  ESL_ALLOC(om2->tfv_mem, sizeof(float32x4_t) * nqf  * p7O_NTRANS +15);

  ESL_ALLOC(om2->rbv, sizeof(uint8x16_t  *) * abc->Kp);
  ESL_ALLOC(om2->sbv, sizeof(uint8x16_t  *) * abc->Kp);
  ESL_ALLOC(om2->rwv, sizeof(int16x8_t   *) * abc->Kp);
  ESL_ALLOC(om2->rfv, sizeof(float32x4_t *) * abc->Kp);

  /* align vector memory on 16-byte boundaries */
  om2->rbv[0] = (uint8x16_t  *) (((unsigned long int) om2->rbv_mem + 15) & (~0xf));
  om2->sbv[0] = (uint8x16_t  *) (((unsigned long int) om2->sbv_mem + 15) & (~0xf));
  om2->rwv[0] = (int16x8_t   *) (((unsigned long int) om2->rwv_mem + 15) & (~0xf));
  om2->twv    = (int16x8_t   *) (((unsigned long int) om2->twv_mem + 15) & (~0xf));
  om2->rfv[0] = (float32x4_t *) (((unsigned long int) om2->rfv_mem + 15) & (~0xf));
  om2->tfv    = (float32x4_t *) (((unsigned long int) om2->tfv_mem + 15) & (~0xf));

  /* copy the vector data */
  memcpy(om2->rbv[0], om1->rbv[0], sizeof(uint8x16_t)  * nqb  * abc->Kp);
  memcpy(om2->sbv[0], om1->sbv[0], sizeof(uint8x16_t)  * nqs  * abc->Kp);
  memcpy(om2->rwv[0], om1->rwv[0], sizeof(int16x8_t)   * nqw  * abc->Kp);
  memcpy(om2->rfv[0], om1->rfv[0], sizeof(float32x4_t) * nqf  * abc->Kp);
...
p7_oprofile_UpdateFwdEmissionScores(P7_OPROFILE *om, P7_BG *bg, float *fwd_emissions, float *sc_arr)
{
  ...
  int     K   = om->abc->K;
  int     Kp  = om->abc->Kp;
  union   { float32x4_t v; float x[4]; } tmp; /* used to align and load simd minivectors               */
...
p7_oprofile_UpdateVitEmissionScores(P7_OPROFILE *om, P7_BG *bg, float *fwd_emissions, float *sc_arr)
{
  ...
  int     Kp  = om->abc->Kp;
  int     idx;
  union   { int16x8_t v; int16_t i[8]; } tmp; /* used to align and load simd minivectors            */
...
p7_oprofile_UpdateMSVEmissionScores(P7_OPROFILE *om, P7_BG *bg, float *fwd_emissions, float *sc_arr)
{
  ...
  union   { uint8x16_t v; uint8_t i[16]; } tmp; /* used to align and load simd minivectors           */
...
sf_conversion(P7_OPROFILE *om)
{
  ...
  tmp  = vreinterpretq_u8_s8(vmovq_n_s8((int8_t) (om->bias_b + 127)));
  tmp2 = vreinterpretq_u8_s8(vmovq_n_s8(127));

  for (x = 0; x < om->abc->Kp; x++)
    {
      for (q = 0;  q < nq;                q++) om->sbv[x][q] = veorq_u8(vqsubq_u8(tmp, om->rbv[x][q]), tmp2);
 ...
 //       esl_neon_dump_float(stdout, (esl_neon_128f_t) om->rfv[x][q]);
  ...
  for (j = 0, k = 1, q = 0; q < nq; q++, k++)
      ...
      for (t = p7O_BM; t <= p7O_II; t++) /* this loop of 7 transitions depends on the order in the definition of p7o_tsc_e */
	{
	  ...
	  om->tfv[j++] = esl_neon_expf((esl_neon_128f_t) tmp.v).f32x4;
	}
    }
...
p7_oprofile_GetFwdTransitionArray(const P7_OPROFILE *om, int type, float *arr )
{
  ...
  int i, j;
  union { float32x4_t v; float x[4]; } tmp; /* used to align and read simd minivectors               */
...
{
  int x, q, z, k;
  union { uint8x16_t v; uint8_t i[16]; } tmp; /* used to align and read simd minivectors           */
  int      M   = om->M;    /* length of the query                                          */
  int      K   = om->abc->Kp;
  int      nq  = p7O_NQB(M);     /* segment length; total # of striped vectors needed            */
...
{
  int x, q, z, k;
  union { float32x4_t v; float f[4]; } tmp; /* used to align and read simd minivectors               */
  int      M   = om->M;    /* length of the query                                          */
  int      K   = om->abc->Kp;
  int      nq  = p7O_NQF(M);     /* segment length; total # of striped vectors needed            */
  int cell_cnt = (om->M + 1) * K;
  ...
  for (x = 0; x < K; x++) {
      for (q = 0, k = 1; q < nq; q++, k++) {
        tmp.v = esl_neon_logf((esl_neon_128f_t) om->rfv[x][q]).f32x4;
        for (z = 0; z < 4; z++)
          if (  (K * (k+z*nq) + x) < cell_cnt)
...
oprofile_dump_vf(FILE *fp, const P7_OPROFILE *om)
{
  ...
  int     t;			/* counter over transitions 0..7 = p7O_{BM,MM,IM,DM,MD,MI,II,DD}*/
  int     j;			/* counter in interleaved vector arrays in the profile          */
  union { int16x8_t v; int16_t i[8]; } tmp; /* used to align and read simd minivectors           */
...
p7_oprofile_Compare(const P7_OPROFILE *om1, const P7_OPROFILE *om2, float tol, char *errmsg)
{
  ...
  int q, r, x, y;
  union { uint8x16_t  v; uint8_t c[16]; } a16, b16;
  union { int16x8_t   v; int16_t w[8];  } a8,  b8;
  union { float32x4_t v; float   x[4];  } a4,  b4;
