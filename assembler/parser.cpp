...
MatePair FASTQParser::read() { // STEP 4: read sequence 
	...
	//std::cout << this->seq1->name.s << std::endl;
	//std::cout << this->seq2->name.s << std::endl;
...
void FASTQParser::do_read() {
	if (kseq_read(this->seq1) < 0 || kseq_read(this->seq2) < 0) {
		eof_bit = true;
	}
}
