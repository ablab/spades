...
/* Begin */
static int myfgets(char *l, int max, FILE *f)
{
	int i;
	int done = 0;
	while(!done)
...
static void standardizeSequence(char *seq)
{
	int i = 0;
	while(seq[i])
	{
...
void checkArray(char** array, unsigned int* available, unsigned int used, unsigned int increment)
{
  if (used == *available)
    {
      *array = (char *) realloc(*array, *available + increment);
      *available += increment;
    }
}

static int getFastQRecord(FILE *infile, Record *record)
{
	const int maxline = 500000;
	char line[500000];

	if(!myfgets(line, maxline, infile)) 
	{
		fprintf(stderr, "getFastQRecord: Unexpected end of file\n"); 
		return 0; 
	}

	record->name = (char *)malloc(strlen(line+1)+1);
	strcpy(record->name, line+1);
	...
	record->sequence = (char *)malloc(strlen(line)+1);
	standardizeSequence(line);
	strcpy(record->sequence, line);
	...
	if(line[0] != '+')
	{
		fprintf(stderr, "getFastQRecord: Unexpected line (was expecting +seq_name while got %s)\n", line); 
		return 0;
	}
	if(!myfgets(line, maxline, infile))
	{
	...
	record->quality = (char *)malloc(strlen(line)+1);
	strcpy(record->quality, line);
	return 1;
}

static int getFastARecord(FILE *infile, Record *record)
{
  ...
  standardizeSequence(string);
  record->name = name;
  record->sequence = string;
  record->quality = (char *)malloc(strlen(string)+1);
  for(usedSeq = 0; usedSeq < strlen(string); usedSeq++)
	record->quality[usedSeq] = 126;
  record->quality[usedSeq] = '\0';

  return 1;
}

int getRecord(FILE* infile, Record *record) // returns 0 if end of file is reached.
{
	int c;
	...
	if (infile == NULL)
	{
		fprintf(stderr, "getRecord: Input file is NULL!\n");
		exit(-1);
	}
	...
	if (c == '@')
		return getFastQRecord(infile, record);
	else if (c == '>')
		return getFastARecord(infile, record);
	else
	{
		fprintf(stderr, "getRecord: Input file does not seem to be in FastQ or FastA format!\n");
		exit(-1);
	}
}

void writeRecord(FILE* outfile, Record *record)
{
	if (outfile == NULL)
	{
		fprintf(stderr, "writeRecord: Output file is NULL!\n");
		exit(-1);
	}
	fprintf(outfile, "@%s\n", record->name);
	fprintf(outfile, "%s\n", record->sequence);
...
void parseDataAndReadFiles(char * filename, int argc, char **argv, boolean * double_strand)
{
	int argIndex = 1;
	FILE *outfile = fopen(filename, "w");
	char *origfilename = (char *)malloc(strlen(filename) + 20);
	int filetype = FASTA;
	Category cat = 0;
...
/*		if(symlink(argv[argIndex], origfilename) < 0)
		{
			fprintf(stderr, "Error in making symbolic link...\n");
			exit(-1);
		} */
