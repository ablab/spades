/***************************************************************************
 * Title:          overlapper-all.c
 * Author:         Haixu Tang
 * Created:        Jun. 2002
 * Last modified:  May. 2004
 *
 * Copyright (c) 2001-2004 The Regents of the University of California
 * All Rights Reserved
 * See file LICENSE for details.
 ***************************************************************************/
#include <stdinc.h>
#include <param.h>
#include <extfunc.h>

char	inpfile[100], outfile[100], qualfile[100], overlapfile[100];
int	qualinp;
char	htmlout, caption[2000], ***content;
FILE	*flog;

void initenv(int argc, char **argv);

main(int argc, char **argv)
{
	int	i, j, k, l, m, n, nconf, nk, l1, l2;
	int	num_vertex, num_class, num_edge;
	READTABLE RT;
	char	**score;
	int	pos1, pos2, num;
	char	temp[100];
	int	dist[10];
	INDEX	**aindex, *aidx;
	READINTERVAL	**readinterval;
	ALIGN	**align, *aln, *aln0;
	FILE	*fp;

	readpar();
	random1(&idum);
	initenv(argc, argv);
	set_score();

	if(htmlout)	{
		flog = ckopen("EULER-report.html", "a");
	} else	{
		flog = ckopen("EULER-report.txt", "a");
	}
	if(!htmlout)	{
  		print_text_line(flog, LINE_LENGTH);
		fprintf(flog, "Start overlap detection with OVERLAPPER; Mininum identity: %2f, Minimum overlap length %d\n",
			 MIN_IDENTITY, MIN_OVERLAP);
		if(qualinp)	{
			fprintf(flog, "Quality file considered.\n");
		} else	{
			fprintf(flog, "Quality file not considered.\n");
		}
	} else	{
	       	print_hl(flog);
		sprintf(caption, "Start overlap detection with OVERLAPPER; Mininum identity: %2f, Minimum overlap length %d\n",
			 MIN_IDENTITY, MIN_OVERLAP);
	       	print_line(flog, caption);
		if(qualinp)	{
			print_line(flog, "Quality file considered.\n");
		} else	{
			print_line(flog, "Quality file not considered.\n");
		}
	}

/*	Input the reads (required) */

/*	Input the reads (required) */
	read_fasta_file(inpfile, &RT);

/*	Input the quality values (optional): 
	score = 2,       if quality >= 30
	        1,       if 30 > quality > 20
		0,	 if quality <= 20
	if no quality values are input, all scores are set up
	as 1				*/

	score = init_score(&RT);
	if(qualinp)	{
		read_score_file(qualfile, score, &RT);
	}

/*      detect possible overlaps between reads  */

        readinterval = (READINTERVAL **) ckalloc(2 * RT.num_seq * sizeof(READINTERVAL *));
        n = detectoverlap(RT.src_seq, score, RT.len_seq, RT.num_seq, readinterval);
	if(!htmlout)	{
        	fprintf(flog, "# found overlap seeds: %d\n", n);
	} else	{
        	sprintf(caption, "# found overlap seeds: %d\n", n);
		print_line(flog, caption);
	}

/*	Generate alignment from overlaps	*/

	align = (ALIGN **) ckalloc(2 * RT.num_seq * sizeof(ALIGN *));

	nconf = 0;
	for(i = 0; i < 2 * RT.num_seq; i ++)	{
		align[i] = AlignReads(RT.src_seq, score, RT.len_seq, RT.num_seq, readinterval[i], i);
		while(readinterval[i])	{
			readinterval[i] = free_readinterval(readinterval[i]);
		}

		aln = align[i];
		while(aln)	{
			nconf ++;
			aln = aln -> next;
		}
	}
	free((void **) readinterval);
	if(!htmlout)	{
		fprintf(flog, "Input %d overlap seeds, %d verified by alignment.\n", n, nconf);
	} else	{
		sprintf(caption, "Input %d overlap seeds, %d verified by alignment.\n", n, nconf);
		print_line(flog, caption);
	}
	
	for(i = 0; i < 7; i ++)	{
		dist[i] = 0;
	}
	num = 0;
	fp = ckopen(outfile, "w");
	int alnNumber = 0;
	for(i = 0; i < 2 * RT.num_seq; i ++)	{
		n = size_align(align[i]);
		fwrite(&n, sizeof(int), 1, fp);
		aln = align[i];
		alnNumber = 0;
		while(aln)	{
			if(aln -> reads[1] >= RT.num_seq)	{
				k = aln -> reads[1] - RT.num_seq;
			} else	{
				k = aln -> reads[1];
			}
			//			printf("read %d alignment: %d of %d: to read: %d mm: %d len: %d \n", i, aln->reads[1], aln->mis_match, aln->length);
			fwrite(&(aln -> reads[1]), sizeof(int), 1, fp);
			fwrite(&(aln -> mis_match), sizeof(int), 1, fp);
			fwrite(&(aln -> length), sizeof(int), 1, fp);
			fwrite(aln -> pos[0], sizeof(int), aln -> length, fp);
			fwrite(aln -> pos[1], sizeof(int), aln -> length, fp);
			//			printf("alnlength: %d\n", aln->length);
			if(aln -> length < 6)	{
				dist[aln -> length] ++;
			} else	{
				dist[6] ++;
			}
			if(aln -> length == 2)	{
				l = 0;
				pos1 = aln -> pos[0][0];
				pos2 = aln -> pos[1][0];
				while(pos1 < aln -> pos[0][1] && pos2 < aln -> pos[1][1])	{
					if(RT.src_seq[aln -> reads[0]][pos1] != RT.src_seq[aln -> reads[1]][pos2])	{
						l = 1;
						break;
					}
					pos1 ++;
					pos2 ++;
				}
				if(l == 0)	{
					num ++;
				}
			
			}
			aln = free_align(aln);
			       ++alnNumber;
		}
	}
	fclose(fp);
	print_gaps(flog, dist);
	if(!htmlout)	{
  		print_text_line(flog, LINE_LENGTH);
	} else	{
		print_hl(flog);
	}
	fclose(flog);

/*	Free memory	*/

	free((void **) align);

	for(i = 0; i < 2 * RT.num_seq; i ++)	{
		free((void *) score[i]);
	}
	free((void **) score);
	free_readtable(&RT);
}

void initenv(int argc, char **argv)
{
	int copt;
	int inpseq, outseq;
	extern char *optarg;

	inpseq = qualinp = outseq = 0;
	MIN_OVERLAP = 100;
	htmlout = 0;

	while ((copt=getopt(argc,argv,"i:q:o:d:l:M:u:p:v:w:g:h:H")) != EOF)	{
		switch(copt) {
			case 'i':
			  inpseq = 1;
			  sscanf(optarg,"%s", inpfile);
			  continue;
			case 'q':
			  qualinp = 1;
			  sscanf(optarg,"%s", qualfile);
			  continue;
			case 'd':
			  sscanf(optarg,"%lf", &MIN_IDENTITY);
			  continue;
			case 'w':
			  sscanf(optarg,"%d", &LPAT);
			  continue;
			case 'u':
			  sscanf(optarg,"%d", &match_score);
			  continue;
			case 'g':
			  sscanf(optarg,"%d", &g);
			  continue;
			case 'h':
			  sscanf(optarg,"%d", &h);
			  continue;
			case 'l':
			  sscanf(optarg,"%d", &overlaplen);
			  continue;
			case 'M':
			  sscanf(optarg,"%d", &MIS_SCORE);
			  continue;
			case 'o':
			  outseq = 1;
			  sscanf(optarg,"%s", outfile);
			  continue;
			case 'v':
			  sscanf(optarg,"%d", &SHORTEST_OVERLAP);
			  continue;
			case 'p':
			  sscanf(optarg,"%d", &MIN_OVERLAP);
			  continue;
			case 'H':
			  htmlout = 1;
			  continue;
			default:
			  if(!htmlout)	{
				  printf("overlapper-all -i InpFile [-q qualfile -d MIN_IDENTITY -l overlaplen -m MIS_SCORE -p MIN_OVERLAP] -o outfile\n");
				  printf("-i InpFile: The input file name of reads\n");
				  printf("-l overlaplen: Minmimum overlap length\n");
				  printf("-q qualfile (optional): Quality file\n");
				  printf("-d MIN_IDENTITY (optional): minimum identity for overlaps\n");
				  printf("-o outfile: Output alignment file\n");
				  printf("-p MIN_OVERLAP (optional): minimal overlap length\n");
			  } else	{
				  print_line(flog, "overlapper-all -i InpFile [-q qualfile -d MIN_IDENTITY -l overlaplen -m MIS_SCORE -p MIN_OVERLAP] -o outfile");
				  print_line(flog, "-i InpFile: The input file name of reads");
				  print_line(flog, "-l overlaplen: Minmimum overlap length");
				  print_line(flog, "-q qualfile (optional): Quality file");
				  print_line(flog, "-d MIN_IDENTITY (optional): minimum identity for overlaps");
				  print_line(flog, "-o outfile: Output alignment file");
				  print_line(flog, "-p MIN_OVERLAP (optional): minimal overlap length");
			  }
			  exit(-1);
		}
		optind--;
	}

	if(inpseq == 0 || outseq == 0)	{
		if(!htmlout)	{
			printf("overlapper-all -i InpFile [-q qualfile -d MIN_IDENTITY -l overlaplen -m MIS_SCORE -p MIN_OVERLAP] -o outfile\n");
			printf("-i InpFile: The input file name of reads\n");
			printf("-l overlaplen: Minmimum overlap length\n");
			printf("-q qualfile (optional): Quality file\n");
			printf("-d MIN_IDENTITY (optional): minimum identity for overlaps\n");
			printf("-o outfile: Output alignment file\n");
			printf("-p MIN_OVERLAP (optional): minimal overlap length\n");
		} else	{
			print_line(flog, "overlapper-all -i InpFile [-q qualfile -d MIN_IDENTITY -l overlaplen -m MIS_SCORE -p MIN_OVERLAP] -o outfile");
			print_line(flog, "-i InpFile: The input file name of reads");
			print_line(flog, "-l overlaplen: Minmimum overlap length");
			print_line(flog, "-q qualfile (optional): Quality file");
			print_line(flog, "-d MIN_IDENTITY (optional): minimum identity for overlaps");
			print_line(flog, "-o outfile: Output alignment file");
			print_line(flog, "-p MIN_OVERLAP (optional): minimal overlap length");
		}	
		exit(-1);
	}
}
