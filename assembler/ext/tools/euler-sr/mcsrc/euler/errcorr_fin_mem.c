...
main(int argc, char **argv)
{
	...
	char	**multip;
	int	dist[10][10], nodedist[20];
	int	intv;
	int	*minc, *maxc, *index;
	int	min1, max1, min2, max2, p, s1, s2, b1, e1, l1, l2, r, num_r, nbad;
	char	**score;
	char	temp[100];
	int	n_match, n_mis, n_indel;
	int 	reads[2];
	ALIGN	**eq_class, *align, *align0, *aln;
	FILE	*fp, *fp1, *fp2, *fp3;
	...
	if(!htmlout)	{
		...
		printf("Output error corrected reads.\n\n");
	} else	{
		/* Commented out print_hl -Vagisha */
	        /* print_hl(flog); */
		/* new function for section head -Vagisha */
	        /* print_line(flog, "Summary of final reads"); */
		print_section_head(flog, "Summary of final reads");
	}
...
/*	Output error corrected/trimmed reads	*/

	for(i = 0; i < 8; i ++)	{
		nodedist[i] = 0;
	}
	minc = (int *) ckalloc(RT.num_seq * sizeof(int));
	maxc = (int *) ckalloc(RT.num_seq * sizeof(int));
	index = (int *) ckalloc(RT.num_seq * sizeof(int));
	num_r = 0;
	for(i = 0; i < RT.num_seq; i ++)	{
		for(j = 0; j < RT.len_seq[i]; j ++)	{
			if(multip[i][j] >= LOW_COV)	break;
		}
		minc[i] = j;
		for(j = RT.len_seq[i] - 1; j >= 0; j --)	{
			if(multip[i][j] >= LOW_COV)	break;
		}
		maxc[i] = j;
		if(maxc[i] - minc[i] > 100)	{
			index[i] = num_r;
			num_r ++;
		} else	{
			index[i] = -1;
...
/*	Write multiplicities	*/
			...
			fprintf(fp2, ">%s\n", RT.src_name[i]);
			for(j = minc[i]; j <= maxc[i]; j ++)	{
				if(multip[i][j] <= LOW_COV)	{
					nbad ++;
				}
				if(multip[i][j] >= 9)	{
					fprintf(fp2, "9");
				} else	{
					fprintf(fp2, "%d", multip[i][j]);
				}
				if((j - minc[i]) % 60 == 59)	{
					fprintf(fp2, "\n");
				}
	...
	fclose(fp2);
	if(!htmlout)	{
		fprintf(flog, "# error corrected reads output: %d.\n", num_r);
	} else	{
		sprintf(caption, "# error corrected reads output: %d.", num_r);
		print_line(flog, caption);
	}

/*	Write overlap alignments	*/
	...
	for(i = 0; i < 2 * RT.num_seq; i ++)	{
		if(i >= RT.num_seq)	{
			p = i - RT.num_seq;
			min2 = (RT.len_seq[p] - maxc[p] - 1);
			max2 = (RT.len_seq[p] - minc[p] - 1);
		} else	{
			p = i;
			min2 = minc[p];
			max2 = maxc[p];
		...
		if(index[p] >= 0)	{
			...
			while(aln)	{
				if(aln -> reads[1] >= RT.num_seq)	{
					...
					min1 = (RT.len_seq[k] - maxc[k] - 1);
					max1 = (RT.len_seq[k] - minc[k] - 1);
				} else	{
					k = aln -> reads[1];
					min1 = minc[k];
					max1 = maxc[k];
				...
				if(index[k] < 0)	{
					...
					for(j = 0; j < aln -> length; j ++)	{
						if(aln -> pos[0][j] >= min2 && aln -> pos[1][j] >= min1) break;
					}
					s1 = j;
					for(j = aln -> length - 1; j >= 0; j --)	{
						if(aln -> pos[0][j] < max2 && aln -> pos[1][j] < max1) break;
					}
					s2 = j;
					if(s1 > 0)	{
						b1 = s1 - 1;
						l1 = min2 - aln -> pos[0][b1];
						l2 = min1 - aln -> pos[1][b1];
						l = max(l1, l2);
						aln -> pos[0][b1] += l;
						aln -> pos[1][b1] += l;
					} else	{
					...
					if(s2 < aln -> length - 1)	{
						e1 = s2 + 1;
						l1 = aln -> pos[0][e1] - max2;
						l2 = aln -> pos[1][e1] - max1;
						l = max(l1, l2) - 1;
						aln -> pos[0][e1] -= l;
						aln -> pos[1][e1] -= l;
					} else	{
					...
					aln -> length = 0;
					for(r = b1; r <= e1; r ++)	{
						if(aln -> pos[0][r] > max2 + 1 ||
						   aln -> pos[1][r] > max1 + 1)	{
							break;
						}
						aln -> pos[0][aln -> length] = aln -> pos[0][r] - min2;
						aln -> pos[1][aln -> length] = aln -> pos[1][r] - min1;
						aln -> length ++;
					}
					l = min(aln -> pos[0][aln -> length - 1] - aln -> pos[0][0],
					        aln -> pos[1][aln -> length - 1] - aln -> pos[1][0]);
					if(aln -> length <= 1 && l <= overlaplen)	{
						if(aln == eq_class[i])	{
							eq_class[i] = aln -> next;
						}
						aln = free_align(aln);
			...
			fwrite(&n, sizeof(int), 1, fp3);
			aln = eq_class[i];
			while(aln)	{
				if(aln -> reads[0] >= RT.num_seq)	{
					aln -> reads[0] = index[aln -> reads[0] - RT.num_seq] + num_r;
				} else	{
					aln -> reads[0] = index[aln -> reads[0]];
				}
				if(aln -> reads[1] >= RT.num_seq)	{
					k = aln -> reads[1] - RT.num_seq;
					aln -> reads[1] = index[k] + num_r;
				} else	{
					k = aln -> reads[1];
					aln -> reads[1] = index[k];
				}
				fwrite(&(aln -> reads[1]), sizeof(int), 1, fp3);
				fwrite(&(aln -> mis_match), sizeof(int), 1, fp3);
				fwrite(&(aln -> length), sizeof(int), 1, fp3);
				fwrite(aln -> pos[0], sizeof(int), aln -> length, fp3);
				fwrite(aln -> pos[1], sizeof(int), aln -> length, fp3);
				aln = aln -> next;
			}
		}
	}
	...
	if(!htmlout)	{
  		print_text_line(flog, LINE_LENGTH);
		fprintf(flog, "Distribution of bad read positions in a single read.\n");
  		print_text_line(flog, LINE_LENGTH);
		fprintf(flog, "# reads     ");
		for(i = 0; i <= 7; i ++)	{
			if(i == 7)	fprintf(flog, ">6       ");
			else		fprintf(flog, "%-9d", i);
		}
		fprintf(flog, "\n");
		fprintf(flog, "    ");
		for(i = 0; i <= 7; i ++)	{
			fprintf(flog, "%9d", nodedist[i]);
		}
		fprintf(flog, "\n");
  		print_text_line(flog, LINE_LENGTH);
	} else	{
		sprintf(caption, "Distribution of bad read positions in a single read.");
		/* changed 2nd arg to allocate content from 9 to 8 -Vagisha*/
		content = allocate_content(2, 8, 50);
		strcpy(content[0][0], "# bad positions");
		strcpy(content[1][0], "# reads");
		for(i = 0; i < 7; i ++)	{
			sprintf(content[0][i + 1], "%d", i);
			sprintf(content[1][i + 1], "%d", nodedist[i]);
		}
		strcpy(content[0][7], ">6");
		/* changed 2nd arg to print_table from 9 to 8 -Vagisha*/
		print_table(flog, 2, 8, content, caption);
		content = free_content(content, 2, 9);
	}
