...
RunCmd::RunCommand("mkdir $workingDir");
...
if ($fileType == $fastQ) {
		RunCmd::RunCommand("$srcDir/assembly/$machtype/qualityTrimmer -fastQ $readsFile -outFasta $workingDir/$readsFile.fasta -minQual 15 -span 3");
		$readsFile = "$workingDir/$readsFile.fasta";
		$altReadFileCreated = 1;
}

if ($qualityFilter != 0) {
		RunCmd::RunCommand("$srcDir/assembly_utils/FilterReads.pl $readsFile $srcDir/assembly/readtitle.rules $readsFile.filtered -maxA 0.85 -maxC 0.85");
		$readsFile = "$readsFile.filtered";
		$altReadFileCreated = 1;
}

if ($altReadFileCreated == 0) {
		RunCmd::RunCommand("ln -s ../$readsFile $workingDir/$readsFile");
		$readsFile = "$workingDir/$readsFile";
}


RunCmd::RunCommand("mkdir $workingDir/Simple");
RunCmd::RunCommand("$srcDir/assembly/$machtype/integralCountSpectrum $readsFile $vertexSize $readsFile.ispect -binary -printCount $trimEndOpt");

my $cmd = "$srcDir/assembly/$machtype/estErrorDist $readsFile.ispect -binary";
print "$cmd\n";
my $errEqCorr = `$cmd`;
...
RunCmd::RunCommand("$srcDir/assembly/$machtype/sortIntegralTupleList $readsFile.ispect -printCount -minMult 2");

RunCmd::RunCommand("$srcDir/assembly/$machtype/fixErrors $readsFile $readsFile.ispect $vertexSize $fixedFile -maxScore 10 -edgeLimit 3 -minVotes 2 -minMult $errEqCorr $maxTrimOpt  ");

system("$srcDir/assembly/assemble.pl $fixedFile $vertexSize");
...
RunCmd::RunCommand("$srcDir/assembly/$machtype/simplifyGraph $fixedFile $workingDir/Simple/$origReadsFile.fixed.simple -minEdgeLength $minEdgeLength -removeSimpleBulges $bulgeSize -removeBulges $bulgeSize -vertexSize $vertexSize -removeLowCoverage 5.0 2");

if ($tryBeta) {
		system("mkdir $workingDir/Transformed");
		RunCmd::RunCommand("$srcDir/assembly/$machtype/transformGraph $workingDir/Simple/$origReadsFile.fixed.simple $vertexSize $workingDir/Transformed/$origReadsFile.fixed.simple.t -notStrict");
		if (-e "$workingDir/Transformed/$origReadsFile.fixed.simple.t.edge") {
				RunCmd::RunCommand("$srcDir/assembly/$machtype/printContigs $workingDir/Transformed/$origReadsFile.fixed.simple.t");
				RunCmd::RunCommand("mv $workingDir/Transformed/$origReadsFile.fixed.simple.t.contig $origReadsFile.contig");
		}
		else {
			RunCmd::RunCommand("$srcDir/assembly/$machtype/printContigs $workingDir/Simple/$origReadsFile.fixed.simple");
			RunCmd::RunCommand("mv $workingDir/Simple/$origReadsFile.fixed.simple.contig $origReadsFile.contig");
		}
}
else {
		RunCmd::RunCommand("$srcDir/assembly/$machtype/printContigs $workingDir/Simple/$origReadsFile.fixed.simple");
