...
int main(int argc, char* argv[]) {
  ...
  for (a = 0; a < lavFile.alignments.size(); a++) {
	...
	for (cb = 1; cb < block->size(); cb++) {
	  ...
	  /*
	    std::cout << refName << " " << qryName << " " << refGapLen << " " << qryGapLen 
	    << " " << block->refALBegin[cb] << " " << block->refALEnd[cb-1];
	    if (qryGapLen > 0 && refGapLen > 0) {
	    std::cout << " " <<  double(refGapLen) / qryGapLen << " " << double(qryGapLen) / refGapLen;
	    } 
	    std::cout << std::endl;
	  */
	  if (refGapLen > minGapLen and 
	      MuchGreater(refGapLen, qryGapLen, muchGreater)  and  
	      MuchGreater(qryGapLen, refGapLen, muchGreater) 
	      and refGapLen > 100 and refGapLen < 10000 ) {
	    if (foundRefName ) 
	      refGapFile << refName << " ";
	    if (foundQryName)
	      refGapFile << qryName << " ";
	    ...
	    refGapFile << " " << block->qryALBegin[cb-1] << " " << block->qryALEnd[cb] << " " 
		       << qryGapLen + 1 << " ";
	    qrySubseq.seq = &qrySeq.seq[block->qryALEnd[cb-1]];
	    qrySubseq.length = qryGapLen+1;
	    qrySubseq.PrintSeq(refGapFile, 0);
	    refGapFile << std::endl;
	  }
	  /*
	    if (qryGapLen > minGapLen and 
	    MuchGreater(qryGapLen, refGapLen, muchGreater) and
	    MuchGreater(refGapLen, qryGapLen, 2 - muchGreater)) {
	    if (foundQryName)
	    qryGapFile << qryName << " ";
	    if (foundRefName ) 
	    qryGapFile << refName << " ";
	...
	if (b > 0) {
	  ...
	  */
	  if (refGapLen > minGapLen and
	      MuchGreater(refGapLen, qryGapLen, muchGreater)  and  
	      MuchGreater(qryGapLen, refGapLen, muchGreater) and 
	      refGapLen < 10000 and refGapLen > 100) {
	    std::cout << "    storing a unaligned sequence " << std::endl;
	    if (foundRefName ) 
	      refGapFile << refName << " ";
	    if (foundQryName)
	      refGapFile << qryName << " ";
