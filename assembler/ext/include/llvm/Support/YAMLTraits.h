...
private:
  ...
  template <typename MK, typename MV, typename Context>
  void processKey(const char *Key, std::map<MK, MV> &Val, bool Required, Context &Ctx) {
     void *SaveInfo;
     bool UseDefault;
     if ( this->preflightKey(Key, Required, false, UseDefault, SaveInfo) ) {
      ...
      if (this->outputting()) {
        for (auto &entry : Val) {
          ...
          this->processKey(Str.data(), entry.second, Required, Ctx);
        }
      } else {
        for (StringRef &key : this->keys()) {
          MK entry;
          StringRef Result = ScalarTraits<MK>::input(key, this->getContext(), entry);
          if (!Result.empty())
            this->setError(llvm::Twine(Result));
          else
            this->processKey(key.data(), Val[entry], Required, Ctx);
        }
      }
...
struct ScalarTraits<unsigned long> {
  static void output(const unsigned long &, void*, raw_ostream &);
...
struct ScalarTraits<unsigned long long> {
  static void output(const unsigned long long &, void*, raw_ostream &);
