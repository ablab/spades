/* Posterior decoding algorithms; NEON versions.
 *
 * Contents:
  ...
  for (i = 1; i <= L; i++)
    {
      ...
      fv    = oxf->dpf[i];
      bv    = oxb->dpf[i];
      totrv = vmovq_n_f32(scaleproduct * oxf->xmx[i*p7X_NXCELLS+p7X_SCALE]);

      for (q = 0; q < Q; q++)
	{
	  /* M */
	  *ppv = vmulq_f32(*fv,  *bv);
	  *ppv = vmulq_f32(*ppv,  totrv);
	  ppv++;  fv++;  bv++;

	  /* D */
    *ppv = vmovq_n_f32(0.0f);
	  ppv++;  fv++;  bv++;
