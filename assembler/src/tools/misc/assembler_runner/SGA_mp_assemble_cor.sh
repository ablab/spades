...
RUNNER_PARAM_SGA_BASE/sga preprocess --pe-mode 1 -o reads.ec.fastq frag1 frag2
...
bwa aln -t $CPU ctg.fasta jump1 > jump1.sai
bwa aln -t $CPU ctg.fasta jump2 > jump2.sai
bwa sampe ctg.fasta jump1.sai jump2.sai jump1 jump2 > jump.sam
samtools view -Sb jump.sam > libMP.bam
...
RUNNER_PARAM_SGA_BASE/sga-bam2de.pl -n $MIN_PAIRS --prefix libMP libMP.bam
RUNNER_PARAM_SGA_BASE/sga scaffold -m 200 -a libPE.astat -o scf --pe libPE.de --mate-pair libMP.de ctg.fasta
