...
struct AlignedCharArray {
#if __GNUC__ && !__has_feature(cxx_alignas) && !GNUC_PREREQ(4, 8, 0)
...
struct AlignedCharArrayUnion : AlignedCharArray<
    alignof(AlignerImpl<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10>),
    sizeof(SizerImpl<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10>)> {
};
...
class ErrorOr {
  ...
  using wrap = detail::ReferenceStorage<typename std::remove_reference<T>::type>;
...
private:
  ...
  union {
    detail::AlignedCharArrayUnion<storage_type> TStorage;
    detail::AlignedCharArrayUnion<std::error_code> ErrorStorage;
  };
  bool HasError : 1;
};
