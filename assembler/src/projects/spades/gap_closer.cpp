...
void GapClosing::run(graph_pack::GraphPack &gp, const char *) {
    ...
    for (const auto& lib : cfg::get().ds.reads.libraries()) {
        if (lib.type() != io::LibraryType::PairedEnd and
            lib.type() != io::LibraryType::Clouds10x and
            lib.type() != io::LibraryType::TellSeqReads)
            continue;
    ...
    for (size_t i = 0; i < dataset.reads.lib_count(); ++i) {
        if (dataset.reads[i].type() != io::LibraryType::PairedEnd and
            dataset.reads[i].type() != io::LibraryType::Clouds10x and
            dataset.reads[i].type() != io::LibraryType::TellSeqReads)
            continue;
