...
namespace debruijn_graph {
    ...
    void Process10XLibrary(debruijn_graph::conj_graph_pack &graph_pack, const lib_t& lib_10x) {
        ...
        while (!stream.eof()) {
            ...
            if (barcode_string != "") {
                if (barcode_string != current_barcode && !paths.empty()) {
                    ...
                    if (paths.size() > cfg::get().pe_params.param_set.rna_10x.min_cloud_size && graph_pack.barcode_indices[0].size() < 70000)
                    ...
                    for (auto path : long_reads_temp_container) {
                            ...
                            long_reads_temp_storage.AddPath(path.first->ToVector(), 1, true, current_barcode, graph_pack.barcode_coverage[0].GetLeftMostPosition(path.first->ToVector()[0], current_barcode),
                                                            graph_pack.barcode_coverage[0].GetRightMostPosition(path.first->ToVector().back(), current_barcode));
                    }
        ...
        path_extend::ContigWriter writer_short(graph_pack.g, std::make_shared<path_extend::DefaultContigNameGenerator>());
        writer.OutputPathsRNA(long_reads, cfg::get().output_dir + "/extracted.fasta");
        writer_short.OutputPathsRNA(long_reads, cfg::get().output_dir + "/extracted_short.fasta");
    }
