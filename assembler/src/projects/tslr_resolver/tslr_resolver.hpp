...
namespace spades {
    ...
    public:
        ...
        void run(debruijn_graph::conj_graph_pack &graph_pack, const char *) {
            ...
            INFO("Average barcode coverage before: " + std::to_string(graph_pack.barcode_mapper->AverageBarcodeCoverage().first) +
            ...
            INFO("Average barcode coverage after: " + std::to_string(graph_pack.barcode_mapper->AverageBarcodeCoverage().first) +
                         ' ' + std::to_string(graph_pack.barcode_mapper->AverageBarcodeCoverage().second));
            std::static_pointer_cast<tslr_resolver::TrimmableBarcodeMapper>(graph_pack.barcode_mapper)->
