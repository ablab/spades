...
static std::vector<EdgeId> CollectDeadEnds(const GraphComponent<Graph> &gc, bool canonical_only = true) {
    const auto &g = gc.g();
...
public:
    ...
    GraphComponent<Graph> Expand(const GraphComponent<Graph> &component) const {
        ...
        return GraphComponent<Graph>::FromEdges(g_,
...
static void WriteComponentWithDeadends(const GraphComponent<Graph> &component,
