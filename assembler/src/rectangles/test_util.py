...
class TestUtils(object):
  ...
  print "diag support", diag.support(), "len", diag.offsetc- diag.offseta"""
    ...
    def should_join(self, diag1, diag2):
        ...
        if len(self.__get_ref_info(e1_1.eid)) == 0 or len(self.__get_ref_info(e1_2.eid)) == 0 or len(
            self.__get_ref_info(e2_1.eid)) == 0 or len(self.__get_ref_info(e2_2.eid)) == 0:
            ...
            for e12_ref_info in self.__get_ref_info(e1_2.eid):
                for e21_ref_info in self.__get_ref_info(e2_1.eid):
                    for e22_ref_info in self.__get_ref_info(e2_2.eid):
                        ...
                        if abs(e11_ref_info) + offsetc_1 == abs(e21_ref_info) + offseta_2 and abs(
                            e12_ref_info) + offsetd_1 == abs(e22_ref_info) + offsetb_2:
                            return True
    ...
    def stat(self):
        self.logger.info(
            "unaligned " + str(self.unaligned) + " true_diags " + str(self.true_diags) + " not_true_diags " + str(
                self.not_true_diags) + " join correct " + str(self.join_correct) + " join incorrect" + str(
                self.join_incorrect) + " join unaligned " + str(self.join_unaligned))

    def have_ref(self, e_first, e_prev, e_cur, pos):
        if len(self.__get_ref_info(e_first.eid)) == 0 or len(self.__get_ref_info(e_prev.eid)) == 0 or len(
            self.__get_ref_info(e_cur.eid)) == 0:
            return False
            ...
            for e_prev_ref in self.__get_ref_info(e_prev.eid):
                for e_cur_ref in self.__get_ref_info(e_cur.eid):
                    if e_first_ref * e_prev_ref < 0 or e_prev_ref * e_cur_ref < 0:
                        continue
                        if abs(e_cur_ref) == abs(e_prev_ref) + e_prev.len and abs(e_cur_ref) == abs(
                            e_first_ref) + pos + e_first.len:
                            return True
        return False
