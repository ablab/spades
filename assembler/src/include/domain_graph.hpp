...
namespace nrps {
    ...
    enum VertexType {
        ADOMAIN, TEDOMAIN, ATDOMAIN, CSTARTDOMAIN, KSDOMAIN, KRDOMAIN, NONE
    };
    static vector<string> types{"ADOMAIN", "TEDOMAIN", "ATDOMAIN", "CSTARTDOMAIN", "KSDOMAIN", "KRDOMAIN", "NONE"};
    ...
    public:
         ...
         */
        Vertex(const std::set<EdgeId> &unique_a_edges, const std::vector<EdgeId> &domain_edges,
               size_t start_coord, size_t end_coord, std::string type, const std::string &name = "")
                : name_(name), unique_domain_edges_(unique_a_edges), domain_edges_in_row_(domain_edges),
                  start_coord_(start_coord), end_coord_(end_coord), v_type_(type), near_to_the_end_of_contig_(true),
                  near_to_the_start_of_contig_(true), visited_(false), visited_times_(0), max_visited_(1) {
        }
        ...
        Vertex(const Vertex &other)
                : name_(other.name_), edges_(other.edges_), unique_domain_edges_(other.unique_domain_edges_),
                  domain_edges_in_row_(other.domain_edges_in_row_), start_coord_(other.start_coord_),
                  end_coord_(other.end_coord_), rc_(other.rc_), v_type_(other.v_type_),
                  ...
                  visited_(other.visited_), visited_times_(other.visited_times_), max_visited_(other.max_visited_) {
        }
    ...
    public:
        ...
        Edge(std::shared_ptr<Vertex> start = nullptr, std::shared_ptr<Vertex> end = nullptr,
             const std::vector<EdgeId> &edges_on_graph = std::vector<EdgeId>(), bool isstrong = false)
                : start_(start), end_(end), visited_(false), strong_(isstrong), length_(0),
                  edges_(edges_on_graph) {
        }
    ...
    public:
        ...
        size_t StrongEdgeCount(std::shared_ptr<nrps::Vertex> v) const {
            return std::count_if(v->edges_.begin(), v->edges_.end(), [](Edge *e) { return e->strong_; });
        }
        ...
        Edge *addEdge(const std::string &v1, const std::string &v2, bool strong, size_t length = 0,
                      const vector<EdgeId> &edges = vector<EdgeId>()) {
            return this->addEdge(getVertex(v1), getVertex(v2), strong, length, edges);
        }

        Edge *addEdge(std::shared_ptr<Vertex> v1, std::shared_ptr<Vertex> v2, bool strong, size_t length = 0,
                      const vector<EdgeId> &edges = vector<EdgeId>()) {
        ...
        std::shared_ptr<Vertex> addVertex(const std::string &name, const std::vector<debruijn_graph::EdgeId> &a_edges,
            ...
            for (auto edge : a_edges) {
                ...
                if (std::find(a_edges_map_[edge].begin(), a_edges_map_[edge].end(), name) == a_edges_map_[edge].end()) {
                    a_edges_map_[edge].push_back(name);
                }
            }
            return this->addVertexInternal(name, unique_edges, a_edges, start_coord, end_coord, type);
        }
        ...
        std::set<std::shared_ptr<Vertex>> const &getNodeSet() const {
            return nodes_;
        }
        ...
        void ExportToDot(const std::string &output_path) const {
            ...
            for (auto v : nodes_) {
                out << "\"" << v->name_ << "\"" << " [label=\"" << v->v_type_ << " " << v->name_
                    << " " << v->max_visited_ << "\"];" << std::endl;
            }

            for (auto e : arcs_) {
                out << "\"" << e->start_->name_ << "\"" << " -> " << "\"" << e->end_->name_ << "\"";
                out << " [label=";
                out << e->length_;
                if (e->strong_) {
        ...
        void makeRC(const std::string &first_vertex, const std::string &second_vertex) {
        ...
        void FindBasicStatistic(std::ofstream &stat_stream) {
            ...
            for (auto domain_type : domain_count) {
                stat_stream << "# " << domain_type.first << "-domains - " << domain_type.second << std::endl;
            }
            stat_stream << std::endl;
        }

        void FindDomainOrderings(debruijn_graph::conj_graph_pack &gp, std::string output_filename) {
            ...
            for (auto v : nodes_) {
                if (nodes_with_incoming.find(v) == nodes_with_incoming.end() && v->edges_.size()) {
            ...
            for (auto v : start_nodes) {
                if (!v->visited_) {
                    ...
                    for (auto vec : answer) {
                        ...
                        path_extend::BidirectionalPath *p = new path_extend::BidirectionalPath(gp.g);
                        std::string outputstring = PathToSequence(p, vec);
                        path_extend::BidirectionalPath *conjugate = new path_extend::BidirectionalPath(p->Conjugate());
        ...
        void ExportPaths(debruijn_graph::conj_graph_pack &gp, std::string output_dir) {
    ...
    private:
        ...
        addVertexInternal(const std::string &name, const std::set<debruijn_graph::EdgeId> &unique_edges,
                          const std::vector<debruijn_graph::EdgeId> &edges,
                          size_t start_coord, size_t end_coord, std::string type) {
            std::shared_ptr<Vertex> node = std::make_shared<Vertex>(unique_edges, edges, start_coord, end_coord, type);
            node->name_ = name;
        ...
        void OutputComponent(debruijn_graph::conj_graph_pack &gp, path_extend::BidirectionalPath *p, int component_id, int ordering_id) {
            auto edges = CollectEdges(p);
            auto comp = GraphComponent<Graph>::FromEdges(gp.g, edges.begin(), edges.end(), true);
            std::ofstream os(cfg::get().output_dir + "/bgc_in_gfa/" + ToString(component_id) + "_" + ToString(ordering_id) + ".gfa");
            path_extend::GFAWriter<Graph> writer(gp.g, os);
            writer.Write(comp);
        }

        set<EdgeId> CollectEdges(path_extend::BidirectionalPath *p) {
            set<EdgeId> edge_set;
        ...
        std::string PathToSequence(path_extend::BidirectionalPath *p, std::vector<std::shared_ptr<Vertex>> &answer) {
            std::stringstream ss;
            ...
            for (size_t i = 0; i < answer.size(); ++i) {
                ...
                for (auto e : v->domain_edges_in_row_) {
                    if (p->Size() == 0 || p->Back() != e) {
                        int gap = 0;
                        if (p->Size() != 0 && p->graph().EdgeEnd(p->Back()) != p->graph().EdgeStart(e)) {
                            gap = 100;
                        }
                ...
                if (i != answer.size() - 1) {
                    ...
                    DEBUG("Translating edge between " << v->name_ << " and " << answer[i + 1]->name_);
        ...
        void OutputStatArrangement(const Graph& g, std::vector<std::shared_ptr<Vertex>> single_candidate, int id, std::ofstream &stat_file) {
            ...
            for (size_t i = 0; i < single_candidate.size(); ++i) {
                ...
                stat_file << v->end_coord_ + current_coord - g.length(p->Back()) << std::endl;
        ...
        void OutputStat(std::set<std::shared_ptr<Vertex>> &preliminary_visited, std::ofstream &stat_file) {
            stat_file << "# domains in the component - ";
            stat_file << preliminary_visited.size() << std::endl;
            int strong_edge_count = std::accumulate(preliminary_visited.begin(), preliminary_visited.end(), 0,
                                                    [&](int prev, std::shared_ptr<Vertex> v) {
                                                        return prev + StrongEdgeCount(v);
                                                    });
            int weak_edge_count = std::accumulate(preliminary_visited.begin(), preliminary_visited.end(), 0,
                                                  [&](int prev, std::shared_ptr<Vertex> v) {
...
//                stat_file << "Warning: small number of domains, possible fragmentation" << std::endl;
            }

            if (strong_edge_count < (int)preliminary_visited.size() / 2) {
//                stat_file << "Warning: small number of strong edges, unlikely to recover complete BGC" << std::endl;
            }
        }

        size_t GetMaxVisited(const Graph& g, std::shared_ptr<Vertex> v, double base_coverage) {
        ...
        void SetCopynumber(const Graph& g, std::set<std::shared_ptr<Vertex>> &preliminary_visited) {
            ...
            for (auto v : preliminary_visited) {
                ...
                if (v->max_visited_ != value) {
                    DEBUG(v->name_ << " copynumber has changed from " << v->max_visited_ << " to " << value);
                }
        ...
        void FindAllPossibleArrangements(const Graph& g,
        ...
        void PrelimDFS(std::shared_ptr<Vertex> v, std::set<std::shared_ptr<Vertex>> &preliminary_visited) {
            for (auto e : v->edges_) {
        ...
        void FinalDFS(std::shared_ptr<Vertex> v, std::vector<std::shared_ptr<Vertex>> &current,
                      std::set<std::shared_ptr<Vertex>> preliminary_visited,
                      std::vector<std::vector<std::shared_ptr<Vertex>>> &answer, size_t component_size) {
            iteration_number_++;
