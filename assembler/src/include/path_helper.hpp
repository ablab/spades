...
inline bool make_dir(std::string const& folder) {
  return mkdir(folder.c_str(), 0755) == 0;
}

inline void remove_dir(std::string const& folder) {
  DIR *dp;
...
inline bool is_regular_file(std::string const& path) {
  struct stat st;
  return (stat(path.c_str(), &st) == 0) && (S_ISREG(st.st_mode));
}

inline std::string append_path(std::string const& prefix, std::string const& suffix) {
...
inline void make_full_path(std::string& path) {
...
inline std::string resolve(std::string const& path) {
...
inline std::string make_relative_path(std::string p, std::string base = current_dir()) {
  p    = resolve(p);
  base = resolve(base);
