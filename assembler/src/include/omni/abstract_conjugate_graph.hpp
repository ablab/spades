...
//          VERIFY(v1 == conjugate(v2));
 ...
 public:
    ...
    pair<VertexId, vector<pair<EdgeId, EdgeId>>> SplitVertex(VertexId v, vector<EdgeId> &splitting_edges, vector<double> &split_coefficients) {
        //TODO:: check whether we handle loops correctly!
        VertexId new_vertex = HiddenAddVertex(v->data());
        vector<pair<EdgeId, EdgeId>> old_2_new_edges(splitting_edges.size());
