	...
	virtual void Count() {
		...
		if (edge_count > 0) {
			INFO(
					...
					<< "% of all edges");
			INFO("Total length of all black edges: " << sum_length << ". While double genome length is " << (2 * cfg::get().reference_genome.size()));
		} else {
			INFO(
...
template<class Graph>
class IsolatedEdgesStat: public omnigraph::AbstractStatCounter {
private:
	typedef typename Graph::EdgeId EdgeId;
	const Graph &graph_;
...
public:
	...
	virtual void Count() {
		...
		for (auto it = graph_.SmartEdgeBegin(); !it.IsEnd(); ++it) {
			EdgeId edge = *it;
			if (graph_.IsDeadEnd(graph_.EdgeEnd(edge)) && graph_.IsDeadStart(
					graph_.EdgeStart(edge)) && black_edges_.count(edge) == 0) {
...
public:
	...
	UniquePathStat(const Graph& g,
			const PairedInfoIndex<Graph>& filtered_index, size_t insert_size,
			size_t max_read_length, double variance_delta) :
		g_(g), filtered_index_(filtered_index), insert_size_(insert_size),
				max_read_length_(max_read_length),
				gap_(insert_size_ - 2 * max_read_length_),
				variance_delta_(variance_delta), considered_edge_pair_cnt_(0),
