...
if len(sys.argv) < 4:
    print "Usage: run_experiment.py <exe-file> <datasets list: E.coli_synth,C.elegans_synth,A.mirum,Rumen>"
    exit(-1)
run_file = sys.argv[1]
...
def extract_params(name, fl):
    ...
    tp = ""
    K = -1
    with open(dataset_info, "r") as fin:
...
def run(exe, params, prefix):
    cmd = exe
    args = str(params[d]["K"]) + " " + params[d]["saves"] + " " + params[d]["reads"] + " " + prefix
    print "Run " + cmd + " " + args
...
if not os.path.exists():
    os.makedirs(resultsdir + "/" + feature)
for d in datasets:
    print "Dataset: ", d
