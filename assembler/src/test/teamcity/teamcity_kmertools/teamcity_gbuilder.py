...
def parse_args():
    ...
    parser.add_argument("--path", "-p", help="custom directory to spades-gbuilder", type=str)
...
def save_run_info(args, output_dir):
    ...
    if args.spades_path:
        run_info.write("path to spades-gbuilder: " + str(args.path) + "\n")
    run_info.close()
...
def make_gbuilder_cmd(args, dataset_info, working_dir, output_dir):
    cmd = os.path.join(working_dir, "spades-gbuilder")
    cmd += " " + dataset_info["dataset_path"]
    cmd += " " + os.path.join(output_dir, "graph")
    if "K" in dataset_info:
        cmd += " -k " + dataset_info["K"]
    return cmd


def run_gbuilder(working_dir, args, dataset_info, output_dir):
    if args.path:
        working_dir = args.path
        log.log("Different spades-gbuilder path specified: " + working_dir)

    cmd = make_gbuilder_cmd(args, dataset_info, working_dir, output_dir)
    log.log("Launching: " + cmd)

    ecode = os.system(cmd)
    if ecode != 0:
        log.err("spades-gbuilder finished abnormally with exit code " + str(ecode))
        return 4
    return 0
...
try:
    ...
    ecode = run_gbuilder(os.path.join(working_dir, "build_spades", "bin"), args, dataset_info, output_dir)
