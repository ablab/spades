...
class PathDistanceEstimator {
 ...
 private:
    const Graph &g_;
    path_extend::validation::ReferencePathIndex reference_path_index_;
...
class PathScaffolderAnalyzer {
 ...
 public:
    PathScaffolderAnalyzer(const conj_graph_pack &gp, size_t long_edge_length_threshold, size_t max_threads);

    PathPairDataset GetFalseNegativeDataset(const PathContainer& paths) const;

 private:
    ScaffoldGraph ConstructScaffoldGraph(const std::set<ScaffoldVertex> &vertices, shared_ptr<BarcodeIndex> index) const;
