...
SearchingExtenderParams DefaultExtenderParamsConstructor::ConstructExtenderParams(size_t lib_index,
    ...
    const auto &paired_index = gp_.get<omnigraph::de::PairedInfoIndicesT<Graph>>("clustered_indices")[lib_index];
    std::shared_ptr<PairedInfoLibrary> paired_lib = MakeNewLib(gp_.get<Graph>(),
                                                               lib,
                                                               paired_index);
...
DefaultExtenderParamsConstructor::DefaultExtenderParamsConstructor(const GraphPack &gp,
