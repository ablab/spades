...
LongEdgePairGapCloserPredicate::LongEdgePairGapCloserPredicate(
    ...
    bool normalize_using_cov, const ScaffoldGraph::ScaffoldEdge& edge) :
    g_(g),
    barcode_extractor_(extractor),
    params_(count_threshold, initial_tail_threshold, check_tail_threshold, score_threshold, normalize_using_cov),
...
bool LongEdgePairGapCloserPredicate::Check(const ScaffoldGraph::ScaffoldVertex& vertex) const {
    ...
    double length_coefficient = static_cast<double>(g_.length(vertex)) / static_cast<double>(params_.length_normalizer_);
...
LongEdgePairGapCloserPredicate::LongEdgePairGapCloserPredicate(const debruijn_graph::Graph& g_,
    ...
    : g_(g_),
      ...
      params_(count_threshold_, initial_tail_threshold, middle_tail_threshold, raw_score_threshold_, normalize_using_cov_),
      start_(start_),
      end_(end_),
      barcodes_(barcodes_) {}
LongEdgePairGapCloserPredicate::LongEdgePairGapCloserPredicate(const debruijn_graph::Graph &g,
    ...
    start_(edge.getStart()),
    end_(edge.getEnd()),
    barcodes_(extractor.GetSharedBarcodesWithFilter(start_, end_, params.count_threshold_, params.length_normalizer_)) {}
...
UniquenessChecker::UniquenessChecker(const ScaffoldingUniqueEdgeStorage &unique_storage_) : unique_storage_(
    unique_storage_) {}
LongEdgePairGapCloserParams::LongEdgePairGapCloserParams(const size_t count_threshold_,
