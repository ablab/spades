...
void ScaffoldGraph::AddVertices(const set<ScaffoldVertex> &vertices) {
    for (auto v : vertices) {
        AddVertex(v);
    }
}
...
void ScaffoldGraph::Print(ostream &os) const {
    ...
    for (auto e_iter = edges_.begin(); e_iter != edges_.end(); ++e_iter) {
        os << "Edge " << e_iter->second.getId() <<
           ...
           << ", length = " << e_iter->second.getLength() << endl;
    }
}
...
vector<ScaffoldGraph::ScaffoldEdge> ScaffoldGraph::IncomingEdges(ScaffoldVertex assembly_graph_edge) const {
...
vector<ScaffoldGraph::ScaffoldEdge> ScaffoldGraph::OutgoingEdges(ScaffoldVertex assembly_graph_edge) const {
    vector<ScaffoldEdge> result;
