...
shared_ptr<scaffold_graph::ScaffoldGraph> PathExtendLauncher::ConstructScaffoldGraph(
...
void PathExtendLauncher::PrintScaffoldGraph(const scaffold_graph::ScaffoldGraph &scaffold_graph,
    ...
    auto vertex_colorer = make_shared<ScaffoldVertexSetColorer>(scaff_vertex_set);
...
void PathExtendLauncher::Launch() {
    ...
    if (cfg::get().ts_res.path_scaffolding_on and params_.pset.sm != sm_old) {
        ...
        DebugOutputPaths(gp_.contig_paths, "loop_traveresed");RemoveOverlapsAndArtifacts(gp_.contig_paths, merged_polished_map, resolver);
