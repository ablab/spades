...
namespace sensitive_aligner_old {
...
template <>
struct hash<sensitive_aligner_old::QueueState> {
    inline size_t hash_size_t_pair(size_t s0, size_t s1) const {
         s1 ^= s1 << 23;  // a
         return (s1 ^ s0 ^ (s1 >> 17) ^ (s0 >> 26)) + s0;
    }

    size_t operator()(const sensitive_aligner_old::QueueState& k) const {
...
class DijkstraGraphSequenceBase {
  public:
    DijkstraGraphSequenceBase(const debruijn_graph::Graph &g,
                              const sensitive_aligner::DijkstraParams &gap_cfg,
                              const std::string &ss,
    ...
    sensitive_aligner::DijkstraReturnCode return_code() const {
        return return_code_;
    }
  ...
  protected:
    ...
    const sensitive_aligner::DijkstraParams gap_cfg_;
    ...
    int min_score_;
    sensitive_aligner::DijkstraReturnCode return_code_;
...
class DijkstraGapFiller: public DijkstraGraphSequenceBase {
  public:
    DijkstraGapFiller(const debruijn_graph::Graph &g,
                      const sensitive_aligner::GapClosingConfig &gap_cfg,
                      const std::string &ss,
...
class DijkstraEndsReconstructor: public DijkstraGraphSequenceBase {
  public:
    DijkstraEndsReconstructor(const debruijn_graph::Graph &g,
                              const sensitive_aligner::EndsClosingConfig &gap_cfg,
                              const std::string &ss,
                              EdgeId start_e, int start_p, int path_max_length)
        : DijkstraGraphSequenceBase(g, gap_cfg, ss, start_e, start_p, path_max_length) {
