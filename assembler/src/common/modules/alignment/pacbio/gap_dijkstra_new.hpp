...
class GapFiller {
        ...
        void Update(QueueState &state, QueueState &prevState, int score)
        {
            // if (g_.int_id(end_e_) == g_.int_id(state.gs.e) ) { 
            //     INFO("Add edge to q " << g_.int_id(end_e_) << " ed=" << score << " pos=" << state.i)
            // }
            if (visited.count(state) > 0) {
                if (visited[state] > score){
        ...
        void AddNewEdge(GraphState gs, QueueState prevState, int ed)
        {
            ...
            string edgestr = tmp.substr(gs.start_pos, gs.end_pos);
            ...
            if (path_max_length_ - ed >= 0){
                ...
                for (int i = 0; i < positions.size(); ++ i) {
                    if (positions[i] >= 0 && scores[i] >= 0) {
                        ...
                        Update(state, prevState, ed + scores[i]);
                    }
                }
    ...
    public:
        ...
        void CloseGap() {
            ...
            while (q.size() > 0) {
                ...
                if (g_.int_id(end_e_) == g_.int_id(curState.gs.e) ) { 
                    INFO("CurState: edgeid=" << g_.int_id(curState.gs.e) << " ind=" << curState.i << " ed=" << ed << " q_size=" << q.size());
                }
                //INFO("EndState: edgeid=" << g_.int_id(end_e_) << " seq_sz=" << ss_.size() << " max_ed=" << path_max_length_);
                if (curState == endqstate) {
                    INFO("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++=Final ed=" << ed);
                    foundPath = true;
                    break;
                }
                ...
                for (const auto &e: g_.OutgoingEdges(g_.EdgeEnd(curState.gs.e))) 
                    if (vs_.count(g_.EdgeEnd(curState.gs.e)) > 0 && vs_.at(g_.EdgeEnd(curState.gs.e)) - (ss_.size() - curState.i) > path_max_length_ - ed ) {
                        ...
                        // if (g_.int_id(end_e_) == g_.int_id(e) ) { 
                        //     INFO("OutState: edgeid=" << g_.int_id(e) << " ed=" << ed << " prev="<< g_.int_id(curState.gs.e) );
                        // }
                        ...
                        if (e == end_e_ && path_max_length_ - ed >= 0){
                            ...
                            if (score != -1) {
                                ...
                                Update(state, curState, ed + score);
                            }
