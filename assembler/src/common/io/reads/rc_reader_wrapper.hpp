...
public:
    ...
    explicit RCRemovingWrapper(typename base::ReadStreamPtrT reader)
...
template<class ReadType>
ReadStream<ReadType> UnRCWrap(ReadStream<ReadType> reader_ptr) {
    return RCRemovingWrapper<ReadType>(std::move(reader_ptr));
}
