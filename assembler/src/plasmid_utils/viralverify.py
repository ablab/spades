...
def naive_bayes(input_list):
    ...
    tr=os.path.dirname(__file__) + "classifier_table_normalized_filtered10.txt"
...
#      if float(i[5]) >= 10 or float(i[5]) <= 0.1:
        hmm_dict[i[0]] = [float(i[4]),float(i[5]),float(i[6])]

# Calculate probabilities for each element of input list
    out_list=[]
    for i in input_list:
        chrom, plasm,vir, chrom_log, plasm_log, vir_log = 1, 1, 1, 0, 0, 0
        for j in i.split():
...
#        if (plasm_log - chrom_log) > threshold: out_list.append(["Plasmid", plasm_log, chrom_log, "{0:.2f}".format(plasm_log - chrom_log)])
 #       else: out_list.append(["Chromosome",  plasm_log, chrom_log, plasm_log - chrom_log])
        if plasm_log > chrom_log and plasm_log > vir_log:
           out_list.append(["Plasmid", plasm_log, chrom_log, vir_log])
        elif chrom_log > plasm_log and chrom_log > vir_log:
           out_list.append(["Chromosome", plasm_log, chrom_log, vir_log])
        else:
           out_list.append(["Virus", plasm_log, chrom_log, vir_log])
    ...
    if args.db:
     for i in ids:
       if i in names_result:
            ...
            final_table.append([i, "No hmm hits", "--"])
       print(final_table)
...
#      print (i)
      if i in names_result:
       final_table.append([i, names_result[i][0][0], names_result[i][0][1:],names_result[i][1]])
      else:
       final_table.append([i, "No hmm hits", "--"])
