...
//		ClipTipsForResolver(resolved_gp.g);
		INFO("ClipTip "<<i);
			...
			//EdgeQuality<Graph> edge_qual(resolved_gp.g, resolved_gp.index, resolved_gp.kmer_mapper, resolved_gp.genome);
			//CompositeLabeler<Graph> labeler(tot_labeler_after, edge_qual);
            //EdgeLocalityPrintingRH<Graph> handler_pict(resolved_gp.g
                    //,labeler
                    //,cfg::get().output_dir
                    //);

            //boost::function<void(typename Graph::EdgeId)> plotter = boost::bind(
                    //&EdgeLocalityPrintingRH<Graph>::HandleDelete,
                    //boost::ref(handler_pict), _1);
...
template<>
void process_resolve_repeats<conj_graph_pack> (conj_graph_pack& origin_gp,
		PairedInfoIndex<conj_graph_pack::graph_t>& clustered_index,
		conj_graph_pack& resolved_gp, const string& graph_name,
		EdgeLabelHandler<conj_graph_pack::graph_t>& labels_after,
		const string& subfolder, bool output_contigs) {
...
//        RectangleResolve(clustered_index, origin_gp.g, cfg::get().output_root + "tmp/", cfg::get().output_dir);
	string postfix = GeneratePostfix();
	typedef TotalLabelerGraphStruct<graph_pack::graph_t> total_labeler_gs;
	typedef TotalLabeler<graph_pack::graph_t> total_labeler;
	total_labeler_gs graph_struct_before(origin_gp.g, &origin_gp.int_ids,
			&origin_gp.edge_pos, NULL);
	total_labeler tot_labeler_before(&graph_struct_before);
	INFO (cfg::get().path_set_graph);
	if (cfg::get().path_set_graph) {
		...
		PathSetGraphConstructor<graph_pack> path_set_constructor(origin_gp,
				clustered_index, resolved_gp);
		path_set_constructor.Construct();
		unordered_map<graph_pack::graph_t::EdgeId,
				graph_pack::graph_t::EdgeId> edge_labels =
				path_set_constructor.GetEdgeLabels();
		labels_after.FillLabels(edge_labels);
		INFO("testing ended");
	} else {
...
//		SaveResolvedPairedInfo(resolved_gp, resolved_graph_paired_info, graph_name + "_resolved", subfolder);
	...
	//Generating paired info for resolved graph
		PairedInfoIndex<graph_pack::graph_t> resolved_cleared_graph_paired_info_before(
				resolved_gp.g);
		if (cfg::get().path_set_graph == false) {
	...
	INFO("---Clearing resolved graph---");
	 ...
	 omnigraph::Compressor<graph_pack::graph_t> compressor(resolved_gp.g);
	    compressor.CompressAllVertices();

	EdgeRemover<graph_pack::graph_t> edge_remover(resolved_gp.g,
			false);
	for (int i = 0; i < 3; ++i) {
//		ClipTipsForResolver(resolved_gp.g);
		INFO("ClipTip "<<i);
	        ...
	        EdgeQuality<Graph> edge_qual(resolved_gp.g, resolved_gp.index, resolved_gp.kmer_mapper, resolved_gp.genome);
	        CompositeLabeler<Graph> labeler(tot_labeler_after, edge_qual);
            string output_folder = cfg::get().output_dir + "repeat_resolver/";
            make_dir(output_folder);
            boost::function<double(Graph::EdgeId)> quality_f = boost::bind(&EdgeQuality<Graph>::quality
                ,boost::ref(edge_qual)
                ,_1);
            cout << quality_f << endl;
            EdgeLocalityPrintingRH<Graph> handler_pict(resolved_gp.g
            ...
            boost::function<void(Graph::EdgeId)> plotter = boost::bind(
                    &EdgeLocalityPrintingRH<Graph>::HandleDelete,
                    boost::ref(handler_pict), _1);
		...
		INFO("Erroneous remove "<<i);
//        BulgeRemoveWrap      (resolved_gp.g);
		FinalRemoveErroneousEdges(resolved_gp.g, edge_remover, plotter);
...
//        RemoveRelativelyLowCoverageEdges(resolved_gp.g);
	...
	//Generating paired info for resolved graph
	PairedInfoIndex<graph_pack::graph_t> resolved_cleared_graph_paired_info(
			resolved_gp.g);
	if (cfg::get().path_set_graph == false) {
