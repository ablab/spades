...
//Never trust a kmer on vertex
					if (int(iter->offset) >= int(cfg::get().K - this->K_) && int(iter->offset) < int(g_.length(iter->edge_id))  )
						res++;
				}
			}
		}
...
//Never trust a kmer on vertex
					if (int(iter->offset) > int(cfg::get().K- this->K_ ) && int(iter->offset) < int(g_.length(iter->edge_id))  )
						res[iter->edge_id].push_back(MappingInstance(iter->offset, size_t(j - K_+ 1), quality));
				}
			}
		}
	}
