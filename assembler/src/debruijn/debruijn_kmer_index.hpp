...
public:
	...
	size_t size() const {
		return data_.size() + push_back_buffer_.size();
	}
...
public:
	...
	void RenewKMers(const Sequence &nucls, IdType id, bool ignore_new_kmers =
			false) {
	...
	void BinWrite(Writer &writer) const {
		...
		writer.write((char*) &sz, sizeof(sz));
		for (size_t i = 0; i < sz; ++i)
			writer.write((char*) &(base::data_[i].count_),
					sizeof(base::data_[0].count_));
		sz = base::push_back_buffer_.size();
		writer.write((char*) &sz, sizeof(sz));
		for (size_t i = 0; i < sz; ++i)
			writer.write((char*) &(base::push_back_buffer_[i].count_),
					sizeof(base::push_back_buffer_[0].count_));
		for (auto it = base::push_back_index_.left.begin(), e =
				base::push_back_index_.left.end(); it != e; ++it) {
			size_t idx = it->second;
			KMer::BinWrite(writer, it->first);
			writer.write((char*) &idx, sizeof(idx));
			sz -= 1;
	...
	void BinRead(Reader &reader, const std::string &FileName) {
		...
		for (size_t i = 0; i < sz; ++i)
			reader.read((char*) &(base::data_[i].count_),
					sizeof(base::data_[0].count_));
		reader.read((char*) &sz, sizeof(sz));
		base::push_back_buffer_.resize(sz);
		for (size_t i = 0; i < sz; ++i)
			reader.read((char*) &(base::push_back_buffer_[i].count_),
					sizeof(base::push_back_buffer_[0].count_));
		for (size_t i = 0; i < sz; ++i) {
			KMer s(base::K_);
			size_t idx;
			...
			base::push_back_index_.insert(
					typename base::KMerPushBackIndexType::value_type(s, idx));
		}
...
template<class IdType, class Seq = runtime_k::RtSeq,
		class traits = kmer_index_traits<Seq> >
class DeBruijnEdgeMultiIndex: public DeBruijnKMerIndex<
		vector<EdgeInfo<IdType> >, Seq, traits> {
...
protected:
	...
	size_t FillBufferFromSequence(const Sequence &seq, KMerBuffer &tmp_entries,
			unsigned num_files) const;
	void DumpBuffers(size_t num_files, size_t nthreads,
			std::vector<KMerBuffer> &buffers, FILE **ostreams) const;

public:
	DeBruijnKMerSplitter(const std::string &work_dir, unsigned K) :
			RtSeqKMerSplitter(work_dir, K) {
	}
};

size_t DeBruijnKMerSplitter::FillBufferFromSequence(const Sequence &seq,
...
# pragma omp parallel for
	for (unsigned k = 0; k < num_files; ++k) {
		...
		libcxx::sort(SortBuffer.begin(), SortBuffer.end(),
				KMerVector<runtime_k::RtSeq>::less2_fast());
		auto it = std::unique(SortBuffer.begin(), SortBuffer.end(),
				KMerVector<runtime_k::RtSeq>::equal_to());

#   pragma omp critical
		{
			fwrite(SortBuffer.data(), SortBuffer.el_data_size(),
					it - SortBuffer.begin(), ostreams[k]);
		}
	}
...
class DeBruijnReadKMerSplitter: public DeBruijnKMerSplitter {
	...
	FillBufferFromStream(ReadStream& stream, KMerBuffer &tmp_entries,
			unsigned num_files, size_t cell_size) const;

public:
	DeBruijnReadKMerSplitter(const std::string &work_dir, unsigned K,
			io::ReadStreamVector<io::IReader<Read> >& streams,
			SingleReadStream* contigs_stream = 0) :
			DeBruijnKMerSplitter(work_dir, K), streams_(streams), contigs_(
					contigs_stream) {
	}
...
std::pair<size_t, size_t> DeBruijnReadKMerSplitter<Read>::FillBufferFromStream(
		ReadStream &stream, KMerBuffer &buffer, unsigned num_files,
...
path::files_t DeBruijnReadKMerSplitter<Read>::Split(size_t num_files) {
	...
	INFO(
			"Splitting kmer instances into " << num_files << " buckets. This might take a while.");
	...
	size_t cell_size = READS_BUFFER_SIZE
			/ (nthreads * num_files * runtime_k::RtSeq::GetDataSize(K_)
					* sizeof(runtime_k::RtSeq::DataType));
...
#   pragma omp parallel for num_threads(nthreads) reduction(+ : counter) shared(rl)
		for (size_t i = 0; i < nthreads; ++i) {
			std::pair<size_t, size_t> stats = FillBufferFromStream(streams_[i],
					tmp_entries[i], num_files, cell_size);
			counter += stats.first;
...
#     pragma omp critical
	...
	if (contigs_) {
		...
		contigs_->reset();
		while (!contigs_->eof()) {
			FillBufferFromStream(*contigs_, tmp_entries[cnt], num_files,
					cell_size);
			DumpBuffers(num_files, nthreads, tmp_entries, ostreams);
			if (++cnt >= nthreads)
				cnt = 0;
...
class DeBruijnGraphKMerSplitter: public DeBruijnKMerSplitter {
	...
	size_t FillBufferFromEdges(EdgeIt &edge, KMerBuffer &tmp_entries,
			unsigned num_files, size_t cell_size) const;

public:
	DeBruijnGraphKMerSplitter(const std::string &work_dir, unsigned K,
			const Graph &g) :
...
template<class Graph>
size_t DeBruijnGraphKMerSplitter<Graph>::FillBufferFromEdges(EdgeIt &edge,
		KMerBuffer &buffer, unsigned num_files, size_t cell_size) const {
	size_t seqs = 0;
	for (size_t kmers = 0; !edge.IsEnd() && kmers < num_files * cell_size;
			++edge) {
...
path::files_t DeBruijnGraphKMerSplitter<Graph>::Split(size_t num_files) {
	...
	size_t cell_size = READS_BUFFER_SIZE
			/ (num_files * runtime_k::RtSeq::GetDataSize(K_)
					* sizeof(runtime_k::RtSeq::DataType));
	INFO("Using cell size of " << cell_size);
	...
	for (auto it = g_.SmartEdgeBegin(); !it.IsEnd();) {
		counter += FillBufferFromEdges(it, tmp_entries[0], num_files,
				cell_size);
...
public:
	...
	size_t BuildIndexFromStream(
			DeBruijnKMerIndex<IdType, runtime_k::RtSeq> &index,
			io::ReadStreamVector<io::IReader<Read> > &streams,
			SingleReadStream* contigs_stream = 0) const {
		DeBruijnReadKMerSplitter<Read> splitter(index.workdir(), index.K(),
				streams, contigs_stream);
		KMerDiskCounter<runtime_k::RtSeq> counter(index.workdir(), splitter);
		KMerIndexBuilder<typename DeBruijnKMerIndex<IdType>::KMerIndexT> builder(
				index.workdir(), 16, streams.size());
		size_t sz = builder.BuildIndex(index.index_, counter, /* save final */
		true);
	...
	void BuildIndexFromGraph(DeBruijnKMerIndex<IdType, runtime_k::RtSeq> &index,
		...
		DeBruijnGraphKMerSplitter<Graph> splitter(index.workdir(), index.K(),
				g);
		KMerDiskCounter<
				typename DeBruijnKMerIndex<typename Graph::EdgeId,
						runtime_k::RtSeq>::KMer> counter(index.workdir(),
				splitter);
		KMerIndexBuilder<
				typename DeBruijnKMerIndex<typename Graph::EdgeId,
						runtime_k::RtSeq>::KMerIndexT> builder(index.workdir(),
				16, 1);
		size_t sz = builder.BuildIndex(index.index_, counter, /* save final */
		true);
...
protected:
	...
	void SortUniqueKMers(KMerCounter &counter, Index &index) const {
		...
		for (auto I = index.kmers->begin(), E = index.kmers->end(); I != E;
				++I) {
...
template<>
class DeBruijnEdgeIndexBuilder<runtime_k::RtSeq> : public DeBruijnKMerIndexBuilder<
		runtime_k::RtSeq> {
...
#     pragma omp critical
	...
	void BuildIndexFromGraph(DeBruijnEdgeIndex<IdType, runtime_k::RtSeq> &index,
		...
		// Now use the index to fill the coverage and EdgeId's
		INFO(
				"Collecting k-mer coverage information from graph, this takes a while.");

		UpdateIndexFromGraph(index, g);
	}

	template<class IdType, class Graph>
	void UpdateIndexFromGraph(
			DeBruijnEdgeIndex<IdType, runtime_k::RtSeq> &index,
			const Graph &g) const {
...
template<>
class DeBruijnEdgeMultiIndexBuilder<runtime_k::RtSeq> : public DeBruijnKMerIndexBuilder<
		runtime_k::RtSeq> {
	typedef DeBruijnKMerIndexBuilder<runtime_k::RtSeq> base;
...
public:
	template<class IdType, class Read>
	size_t BuildIndexFromStream(
			DeBruijnEdgeMultiIndex<IdType, runtime_k::RtSeq> &index,
			io::ReadStreamVector<io::IReader<Read> > &streams,
			SingleReadStream* contigs_stream = 0) const {
		unsigned nthreads = streams.size();
