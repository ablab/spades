...
class LengthThresholdFinder {
 public:
  ...
  static size_t MaxBulgeLength(size_t k, double coeff, size_t additive_coeff) {
...
class ConditionParser {
 private:
  ...
  shared_ptr<Predicate<EdgeId>> ParseCondition(size_t& min_length_bound,
    ...
    if (next_token_ == "tc_lb") {
      ...
      size_t length_bound = LengthThresholdFinder::MaxTipLength(read_length_,
                                                                g_.k(),
                                                                length_coeff);
      ...
      DEBUG(
          "Creating length bound for erroneous connections originated from tip merging. Coeff " << length_coeff);
      ...
      size_t length_bound = LengthThresholdFinder::MaxErroneousConnectionLength(
          g_.k(), length_coeff);
  ...
  shared_ptr<Predicate<EdgeId>> ParseConjunction(size_t& min_length_bound,
                                                 double& min_coverage_bound) {
    shared_ptr<Predicate<EdgeId>> answer = make_shared<AlwaysTrue<EdgeId>>();
    VERIFY(next_token_ == "{");
    ReadNext();
    while (next_token_ != "}") {
      answer = make_shared<AndOperator<EdgeId>>(
          answer, ParseCondition(min_length_bound, min_coverage_bound));
      ReadNext();
    }
    return answer;
  }
 ...
 public:
  ...
  ConditionParser(const Graph& g, string input, size_t read_length,
    ...
    for (auto it = tmp_tokenized_input.begin(); it != tmp_tokenized_input.end();
        ++it) {
  ...
  shared_ptr<Predicate<EdgeId>> operator()() {
    ...
    while (next_token_ == "{") {
      ...
      answer = make_shared<OrOperator<EdgeId>>(
          answer, ParseConjunction(min_length_bound, min_coverage_bound));
...
boost::function<void(typename Graph::EdgeId)> EnableProjection(
    gp_t& gp, boost::function<void(typename Graph::EdgeId)> removal_handler_f) {
  typedef typename Graph::EdgeId EdgeId;
...
bool ClipTipsWithProjection(
    gp_t& gp, const debruijn_config::simplification::tip_clipper& tc_config,
    bool enable_projection = true, size_t read_length = 0,
    double detected_coverage_threshold = 0.,
    boost::function<void(typename gp_t::graph_t::EdgeId)> removal_handler_f = 0,
    size_t iteration_count = 1, size_t iteration = 0) {
  return ClipTips(
      gp.g,
      tc_config,
...
typename omnigraph::BulgeRemover<Graph>::BulgeCallbackF GetBulgeCondition(
  ...
  return boost::bind(
      ...
      omnigraph::SimplePathCondition<ConjugateDeBruijnGraph>(graph), _1, _2);
}

template<class Graph>
bool RemoveBulges(
    Graph& g, const debruijn_config::simplification::bulge_remover& br_config,
...
bool RemoveThorns(
  ...
  return ThornRemover<Graph>(g, max_unr_length, isec_config.uniqueness_length,
                             isec_config.span_distance, removal_handler).Process();
}
...
void SimplifyGraph(conj_graph_pack &gp,
  ...
  //ec auto threshold
  double determined_coverage_threshold =
      FindErroneousConnectionsCoverageThreshold(gp.g, gp.index.inner_index());
  INFO(
...
//	VERIFY(gp.kmer_mapper.IsAttached());
  ...
  size_t max_length = std::max(*cfg::get().ds.RL,
