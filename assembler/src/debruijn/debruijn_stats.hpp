...
void WriteKmerComponent(conj_graph_pack &gp,
	...
	shared_ptr<GraphSplitter<Graph>> splitter = EdgeNeighborhoodFinder<Graph>(gp.g, gp.index.get(kp1mer).first, 50, is);
	shared_ptr<GraphComponentFilter<Graph>> filter = make_shared<ComponentSizeFilter<Graph>>(gp.g, 500, 2, 500);
//	omnigraph::visualization::PathColorer<Graph> colorer(gp.g, path1, path2);
	omnigraph::visualization::WriteComponents<Graph>(gp.g, splitter, filter, folder + "kmer.dot",
...
struct detail_info_printer {
	...
	:
			folder_(folder), func_(
					bind(&ProduceDetailedInfo, boost::ref(gp), boost::ref(labeler),
					     _3, _1, gp.k_value)),
							graph_(gp.g) {
	}
