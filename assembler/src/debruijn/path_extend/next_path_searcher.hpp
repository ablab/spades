...
public:
    Edge(const Graph& g, EdgeId id, Edge* prev_e, size_t dist, int gap = 0) :
            g_(g), id_(id), prev_edge_(prev_e), dist_(dist), gap_(gap)
    {
    ...
    bool IsCycled()
    {
        ...
        bool is_cycled = path.getLoopDetector().IsCycled(
                cfg::get().pe_params.param_set.loop_removal.mp_max_loops,
                identical_edges);
        if (path.Size() > 1
                && path.At(path.Size() - 1) == path.At(path.Size() - 2))
        {
            is_cycled = true;
        }
        return is_cycled;
    ...
    bool EqualBegins(const BidirectionalPath& path, int pos)
    {
        ...
        return path_extend::EqualBegins(path, (size_t) pos, p, p.Size() - 1,
                true);
        /*Edge* curr_edge = this;
...
private:
    void GrowPath(const BidirectionalPath& init_path, Edge* e, size_t max_len,
            vector<Edge*>& to_add);
    Edge* AddPath(const BidirectionalPath& init_path, Edge* prev_e,
            size_t max_len, const BidirectionalPath& path, size_t start_pos);
    Edge* AnalyzeBubble(const BidirectionalPath& p, EdgeId buldge_edge,
            size_t gap, Edge* prev_edge);

    void ScaffoldTip(const BidirectionalPath& path, Edge * current_path,
            vector<Edge*>& result_edges, vector<Edge*>& stopped_paths,
            vector<Edge*>& to_add, bool jump);
    void ScaffoldChristmasTree(const BidirectionalPath& path,
            Edge * current_path, vector<Edge*>& to_add);
    void Scaffold(const BidirectionalPath& init_path, Edge* current_path,
            vector<Edge*>& to_add);
    void FindScaffoldingCandidates(const BidirectionalPath& init_path,
            Edge* current_path, EdgeSet& candidate_set);
    void FindScaffoldingCandidates(EdgeId e, size_t distance_to_tip,
            vector<EdgeWithDistance>& jump_edges);
    void OrderScaffoldingCandidates(EdgeSet& candidate_set,
            const BidirectionalPath& init_path, Edge* current_path,
            vector<Edge*>& to_add);
    void RemoveRedundant(ConstructedPathT& constructed_paths);
    void ProcessScaffoldingCandidate(EdgeWithDistance& e,
            EdgeSet& candidate_set, Edge* current_path, size_t grown_path_len,
    ...
    void AddConstructedPath(const BidirectionalPath& cp, size_t from, int gap,
            ConstructedPathT& constructed_paths);
    void FilterBackPaths(set<BidirectionalPath*>& back_paths,
            EdgeId edge_to_reach, set<BidirectionalPath*>& reached_paths,
            size_t max_len = -1UL);
        ...
        const BidirectionalPath& path, EdgeId begin_edge, bool jump)
{
    ...
    Edge* start_e = new Edge(g_, path.At(0), NULL,
            g_.length(path.At(0)) + path.GapAt(0), path.GapAt(0));
    Edge* e = start_e->AddPath(path, 1);
    if (begin_edge != path.Back())
    {
        e = e->AddOutEdge(begin_edge);
        DEBUG(
                "Try to find next path for path with edge " << g_.int_id(begin_edge));
    }
    else
    {
        DEBUG(
                "Try to search for path with last edge " << g_.int_id(path.Back()) << " Scaffolding: " << jump << ", next edges " << g_.OutgoingEdgeCount(g_.EdgeEnd(path.Back())));
    }
    ...
    while (ipath < grow_paths.size())
    {
        ...
        if (!current_path->IsCorrect()
                || (current_path->IsCycled()
                        && current_path->Length() > init_length)
                || used_edges.count(current_path) > 0)
        {
        ...
        if (to_add.empty())
        {
            ScaffoldTip(path, current_path, result_edges, stopped_paths, to_add,
                    jump);
        }
    ...
    {
        BidirectionalPath result_path = result_edges[i]->GetPrevPath(
                path.Size());
    ...
    delete start_e;
    DEBUG(
            "for path " << path.GetId() << " several extension " << result_paths.size());
    return result_paths;
}

inline Edge* NextPathSearcher::AnalyzeBubble(const BidirectionalPath& p,
        EdgeId buldge_edge, size_t gap, Edge* prev_edge)
{
...
inline Edge* NextPathSearcher::AddPath(const BidirectionalPath& init_path,
        Edge* prev_e, size_t max_len, const BidirectionalPath& path,
        size_t start_pos)
{
    Edge* e = prev_e;
...
inline void NextPathSearcher::GrowPath(const BidirectionalPath& init_path,
        Edge* e, size_t max_len, vector<Edge*>& to_add)
{
    if (!e->IsCorrect())
        return;
...
inline void NextPathSearcher::ScaffoldTip(const BidirectionalPath& path,
        Edge * current_path, vector<Edge*>& result_edges,
        vector<Edge*>& stopped_paths, vector<Edge*>& to_add, bool jump)
{
        ...
        vector<Edge*>& to_add)
{
    ...
    for (Edge* ed : candidates)
    {
        ...
        {
            DEBUG(
                    "Starting new path searcher " << path.Length() << ", " << search_dist_ << " " << ed->Length() << " " << path.Length() + search_dist_ - ed->Length());
            NextPathSearcher further_searcher(g_, cover_map_,
                    path.Length() + search_dist_ - ed->Length(),
                    weight_counter_);
            set<BidirectionalPath*> further_paths =
                    further_searcher.FindNextPaths(ed->GetPrevPath(0),
                            ed->GetId(), true);
        ...
        Edge* current_path, vector<Edge*>& to_add)
{
    ...
    {
        DEBUG(
                "Edge " << g_.int_id(e.e_) << " (" << g_.length(e.e_) << ")" << ", distance " << e.d_);
    }

    OrderScaffoldingCandidates(candidate_set, init_path, current_path, to_add);
}

inline void NextPathSearcher::FindScaffoldingCandidates(
        const BidirectionalPath& init_path, Edge* current_path,
        EdgeSet& candidate_set)
{
    set<EdgeId> path_end;
    ...
    for (std::pair<EdgeId, vector<int> > e : candidates)
    {
        ...
        int avg_distance = 0;
        DEBUG(
                "All distances for edge " << g_.int_id(e.first) << " (" << g_.length(e.first) << ")");
...
inline void NextPathSearcher::FindScaffoldingCandidates(EdgeId e,
        size_t distance_to_tip, vector<EdgeWithDistance>& jump_edges)
{
    if (g_.length(e) < long_edge_len_
            || distance_to_tip - g_.length(e) >= search_dist_)
        return;

    DEBUG("Edge " << g_.int_id(e) << ", length " << g_.length(e));
    DEBUG(
            distance_to_tip << " " << distance_to_tip - g_.length(e) << " " << search_dist_);

    set<EdgeId> candidate_edges;
    int min_distance = (int) distance_to_tip
            - (int) weight_counter_.GetLib().GetLeftVar();
    int max_distance = (int) search_dist_ + (int) g_.length(e);
    DEBUG("Looking in range " << min_distance << " " << max_distance);
    weight_counter_.FindJumpCandidates(e, min_distance, max_distance,
            long_edge_len_, candidate_edges);
    weight_counter_.FindJumpEdges(e, candidate_edges, min_distance,
            max_distance, jump_edges);
...
inline void NextPathSearcher::RemoveRedundant(
        ConstructedPathT& constructed_paths)
{
...
inline void NextPathSearcher::ProcessScaffoldingCandidate(EdgeWithDistance& e,
        EdgeSet& candidate_set, Edge* current_path, size_t grown_path_len,
        ConstructedPathT& constructed_paths)
{

    bool looking_for_tip = (g_.OutgoingEdgeCount(
            g_.EdgeEnd(current_path->GetId())) == 0);

    //Search back from e till tip or maximim length back
    DEBUG(" === Searching back === ");
    DEBUG(
            "Distances: search = " << search_dist_ << ", grown = " << grown_path_len << ", estimated gap = " << e.d_);
    ...
    DEBUG(
            "Searchin for edge of length " << g_.length(e.e_) << " to dist " << max_length_back);
    NextPathSearcher back_searcher(g_, cover_map_, max_length_back,
            weight_counter_);
    BidirectionalPath jumped_edge(g_, g_.conjugate(e.e_));
    set<BidirectionalPath*> back_paths = back_searcher.FindNextPaths(
            jumped_edge, jumped_edge.Back(), false);
    ...
    if (back_paths.empty())
    {
        ...
        {
            DEBUG(
                    "Added tip edge " << g_.int_id(e.e_) << " (" << g_.length(e.e_) << ")" << ", distance " << e.d_);
            constructed_paths.insert(
                    make_pair(
                            e.e_,
                            PathWithDistance(BidirectionalPath(g_, e.e_),
                                    e.d_)));
        }
        else if (g_.IncomingEdgeCount(g_.EdgeStart(e.e_)) > 0
                && !looking_for_tip)
        {
            //No back paths -- just scaffold
            DEBUG(
                    "Added edge with no back path " << g_.int_id(e.e_) << " (" << g_.length(e.e_) << ")" << ", distance " << e.d_);
            constructed_paths.insert(
                    make_pair(
                            e.e_,
                            PathWithDistance(BidirectionalPath(g_, e.e_),
    ...
    else
    {
        ...
        FilterBackPaths(back_paths, g_.conjugate(current_path->GetId()),
                reached_paths, search_dist_ - grown_path_len);
        ...
        if (reached_paths.size() > 0 && !looking_for_tip)
        {
            ...
            for (BidirectionalPath* p : reached_paths)
            {
                ...
                AddConstructedPath(cp, reached_edge_pos + 1, 0,
                        constructed_paths);
            }
        }
        else if (reached_paths.size() > 0 && looking_for_tip)
        {
            FATAL_ERROR("Impossible: back path reaches tip");
        }
        else if (looking_for_tip)
        {
            DEBUG(
                    "Found " << back_paths.size() << " path(s) going back to tip");
            int i = 0;
            for (BidirectionalPath* p : back_paths)
            {
                ...
                //Adding jumped edge since its not included in the path
                cp.PushBack(e.e_);
                AddConstructedPath(cp, 0, EstimateGapForPath(candidate_set, cp),
                        constructed_paths);
            }
        }
    }
...
inline int NextPathSearcher::EstimateGapForPath(EdgeSet& candidate_set,
        const BidirectionalPath& p)
{
    int gap = 0;
    int count = 0;
    for (EdgeWithDistance e : candidate_set)
...
inline void NextPathSearcher::AddConstructedPath(const BidirectionalPath& cp,
        size_t from, int gap, ConstructedPathT& constructed_paths)
{
    ...
    for (auto it = constructed_paths.lower_bound(candidate);
            it != constructed_paths.upper_bound(candidate); ++it)
            {
    ...
    DEBUG("add constructed path " << g_.int_id(candidate));
    cp.Print();
...
inline void NextPathSearcher::FilterBackPaths(
        set<BidirectionalPath*>& back_paths, EdgeId edge_to_reach,
        set<BidirectionalPath*>& reached_paths, size_t max_len)
{
    ...
    for (auto piter = back_paths.begin(); piter != back_paths.end();)  {
        ...
        if (p->FindFirst(edge_to_reach) != -1)
        {
            ...
            ++piter;
                ...
                || (g_.OutgoingEdgeCount(last_v) == 1
                        && (*g_.OutgoingEdges(last_v).begin()) == last_e))
                && p->Length() < max_len)
        {
            ++piter;
        }
...
inline void NextPathSearcher::JoinPaths(ConstructedPathT& constructed_paths)
{
    ...
    for (auto p1 = constructed_paths.begin(); p1 != constructed_paths.end();
        ...
        for (auto p2 = constructed_paths.begin(); p2 != constructed_paths.end();
                ++p2)
                {
            ...
            {
                if (edges_path1.count(path2.At(i)) > 0
                        || edges_path1.count(g_.conjugate(path2.At(i))) > 0)
                {
                    has_pi = false;
                }
            }
