...
public:
    ...
    CompositeGapJoiner(const Graph& g, vector<shared_ptr<GapJoiner>> joiners, size_t may_overlap_threhold, int must_overlap_threhold, size_t artificail_gap) :
            GapJoiner(g), joiners_(joiners), may_overlap_threshold_(may_overlap_threhold), must_overlap_threshold_(must_overlap_threhold), artificial_gap_(artificail_gap)
            {  }

    Gap FixGap(EdgeId source, EdgeId sink, int estimated_gap) const {
