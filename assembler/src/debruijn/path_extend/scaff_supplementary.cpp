...
void ScaffoldingUniqueEdgeAnalyzer::FillUniqueEdgeStorage(ScaffoldingUniqueEdgeStorage &storage_) {
    ...
    for (auto iter = gp_.g.SmartEdgeBegin(); !iter.IsEnd(); ++iter) {
        ...
        total_len += tlen;
        if (gp_.g.length(*iter) >= length_cutoff_ && gp_.g.coverage(*iter) > median_coverage_ * max_relative_coverage_) {
    ...
    INFO("Unique edges quantity: " << unique_len << ", unique edges length " << unique_len <<", total edges length" << total_len);
