...
static const size_t MAX_LOOPS = 5;
static const bool FULL_LOOP_REMOVAL = true;

static const double EDGE_COVERAGE_TRESHOLD = 0.8;
static const double LENGTH_COVERAGE_TRESHOLD = 0.99;
...
void FindPaths(Graph& g, std::vector<BidirectionalPath>& seeds, PairedInfoIndex<Graph>& pairedInfo, std::vector<BidirectionalPath>& paths, bool allSeeds = false) {
...
//		if (!allSeeds && PathsCoverage(g, paths) > EDGE_COVERAGE_TRESHOLD && PathsLengthCoverage(g, paths) > LENGTH_COVERAGE_TRESHOLD) {
//			break;
//		}
	}
