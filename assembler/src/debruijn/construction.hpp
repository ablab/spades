...
void construct_graph(PairedReadStream& stream, conj_graph_pack& gp, graph_labeler& labeler, paired_info_index& paired_index)
{
    ...
    if (cfg::get().paired_mode)
    {
        if (cfg::get().etalon_info_mode)
            ConstructGraphWithEtalonPairedInfo<K>(gp.g, gp.index, paired_index, stream, gp.genome);
        else
            ConstructGraphWithPairedInfo<K>      (gp.g, gp.index, paired_index, stream);

        FillEtalonPairedIndex<K>(gp.g, gp.etalon_paired_index, gp.index, gp.genome);
    }
    else
    {
...
void save_construction(conj_graph_pack& gp, total_labeler& tl, paired_info_index& paired_index)
{
...
void make_construction(PairedReadStream& stream, conj_graph_pack& gp, total_labeler& tl, paired_info_index& paired_index)
{
