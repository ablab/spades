...
void BulgeRemover<Graph>::RemoveBulges(Graph& g) {
	...
	for (EdgeIter iterator = g.SmartEdgeBegin(), end = g.SmartEdgeEnd(); end
		...
		if (PossibleBulgeEdge(g, edge)) {
			...
			DEBUG(
					"Best path with coverage " << path_and_coverage.second
							<< " is " << PrintPath<Graph> (g,
							path_and_coverage.first));

			//if edge was returned, this condition will fail
			DEBUG(
					"Satisfies condition " << (path_and_coverage.second
							> coverage_gap * g.kplus_one_mer_coverage(edge)));
			if (path_and_coverage.second > coverage_gap
					* g.kplus_one_mer_coverage(edge)) {
				DEBUG(
