...
Since each rectangle $R$ is formed by a pair of non-branching substrings $\alpha = a_i\ldots a_{i+t_1}$
and $\beta = a_i^*\ldots a_{i^*i+ t_2}$, it is intersected by the line $(d)$ if and only if 
there exists $x$ such that $i\leq x \leq i+t_1$ and $ i^* \leq x + d \leq i^* + t_2$ (1). This condition
...
Given a $d$-bounded rectangle $R$ formed by a pair of non-branching substrings $\alpha = a_i\ldots a_{i+t_1}$
and $\beta = a_i^*\ldots a_{i^*i+ t_2}$, let $D_R= i^* - i$ be the genomic distance between $\alpha$ and $\beta$. The integer 
...
by mapping read pairs to long edges in the graph. We set $d$ equal to the median of the distribution.  
