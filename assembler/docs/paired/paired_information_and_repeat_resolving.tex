...
\begin{definition}
Directed distance between two vertices $A$ and $B$ in path $P$, started in $A$ and finished in $B$, is the sum of the lengths of all edges in path $P$. 
%\begin{itemize}
...
\begin{definition}
Directed distance between two edges $a$ and $b$ in path $P$ started from edge $a$ and finished by $b$ is sum of the lengths of all edges of $P$ with exception of $b$. 
...
\begin{problem}
  With given two $S$-consistent ungluings of a graph $G$, we want to define which one is better.
\end{problem}
...
Let's consider rectangle graph\footnote{We assume that basic concept of rectangle graph is known by reader.} approach to repeat resolving. 
...
\end{enumerate}
Let's denote the resulting graph after all three steps as $G_r = (V_r,E_r)$ and result of first two steps as $G_{rb} = (V_{rb}, E_{rb})$. 
\begin{remark}
Labels of vertices in rectangle graph can be considered as vertex pair info.
\end{remark}
Each vertex $v_{rb}$ from $V_{rb}$ has a label --- vertex pair info $(A(v_{rb}), B(v_{rb}), x(v_{rb}))$. We call first element of such triple \emph{first mark} of vertex $v_{rb}$ and denote it $\mathop{fm}(v_{rb})$. 
...
\begin{definition}
For given graph $G=(V,E)$ and $d$-set of edge pair info $P$ 
