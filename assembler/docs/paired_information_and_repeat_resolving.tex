...
Next step is to resolve repeats using distances distributions on neighbouring edge (to divide sum of two close normal distributions with one). If coverage is enough we can try to correct information. For example it is possible to obtain information about distance to vertex from distances to two edges. This two series of distances may be same. If not it is possible to find minimal correction to get same distances.    
