...
First approach is modifying adjacency definition for vertex-edge pairs.
\begin{definition}
...
\end{definition}
This definition slightly different from previous adjacency definition. So terms in this definition we can split vertex by vertex-edge information even if there are two edges with common start or finish vertex.   
...
\begin{definition}
 Two vertex-edge pairs $(Z,a,x)$ and $(Z,b,y)$ are called \emph{$l-\eps$ adjacent} if there exists path $P$ from $a$ to $b$: $|P|<l$ and $|P|$ is similar to $y-x$ (difference not exceed $\eps$).   
\end{definition}

\subsection{Milty split operation}

After doing split for all possibilities we can recalculate pair information, and do split again. It is possible that after fist splitting some connected components of pair infos may break into few parts. In this case we can do split operation again. Some time it is possible to split by \emph{back} information. This splitting is rectangle graph operation for contracting red edge. In terms of splitting it is obvious if back splitting for vertex is still possible.  


\subsection{Rectangle graph hard case}

There are simple graph that unaffected by rectangle graph split operation.
Vertices is ${A,B,C,E,F,G,H}$ with edges $\{(A,B),(B,C),(C,E),(E,F),(F,G),(G,A),(C,H),(H,F)\}$. Pair information is set of all edge pairs with real distance 2 i.e. $(A,B)$ and $(C,E)$.
Using of slightly modified split operation for this graph, and after that using back split operation produce two loops $(A\rightarrow B\rightarrow C\rightarrow E\rightarrow F\rightarrow G\rightarrow A)$ and $(A\rightarrow B\rightarrow C\rightarrow H\rightarrow F\rightarrow G\rightarrow A)$. 
