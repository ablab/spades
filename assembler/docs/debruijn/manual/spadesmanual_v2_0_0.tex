...
spades.py
\end{lstlisting}
This runs {\spades} on a toy dataset (first 1,000 bp of {\ecoli}) that {\spades} comes with. If the installation was successfull you will see the following lines in the end of the run.\todo{Lyosha, vsyo verno?}
\begin{lstlisting}
blah
\end{lstlisting}
...
in FASTA{\todo{Lyosha, verno?}} and FASTQ format. The number of files with single reads may be arbitrary while paired end reads should be organized into one or two files. In a single file, each left read should be followed by its mate right read. In case of two files, the first file should contain all left reads while
the second one should contain all the corresponding right reads in exactly the same order.

All the files may be gzipped.

\todo[inline]{Prjibel, skripti?}
...
and then explain its contents in detail.\todo{Lyosha, ya ubral developer mode flag. Nichego?}
...
there is a folder containing statistics on various metrics (like N50) of the resulting contigs.\todo{perepisat' nignii box}
\begin{lstlisting}
