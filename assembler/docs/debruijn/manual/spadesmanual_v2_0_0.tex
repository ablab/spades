...
%If {\spades} runs the error correction stage on the same input reads it has already corrected, it detects this and prompts the user not to correct the reads again.
If you run {\spades} with same {\tt output\_dir} and {\tt project\_name} as in one of previous run of {\spades}, it detects if you already run the error correction stage and prompts the user not to correct the reads again.
...
Note that the quality is estimated even if this section is empty. Our qauality tool uses Plantagora (\url{http://www.plantagora.org/}) and MUMmer (\url{http://mummer.sourceforge.net/}).
