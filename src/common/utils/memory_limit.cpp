...
void limit_memory(size_t limit) {
    rlimit rl;
    if (sizeof(rlim_t) < 8) {
        FATAL_PARAM_ERROR("Can't limit virtual memory because of 32-bit system");
        return;
    }

    int res = getrlimit(RLIMIT_AS, &rl);
    if (res != 0)
        FATAL_IO_ERROR("getrlimit(2) call failed, errno = " << errno);
...
size_t get_used_memory() {
    ...
    // Update statistics cached by mallctl
    {
        ...
        if (je_mallctl("epoch", &epoch, &sz, &epoch, sz) != 0)
            FATAL_IO_ERROR("mallctl() call failed, errno = " << errno);
    }
