...
const std::filesystem::path &TmpDirImpl::release() {
    ...
    CHECK_FATAL_PARAM_ERROR(!already_released, "Temp dir is already released");
    return dir_;
}
...
const std::filesystem::path &TmpFileImpl::release() {
    CHECK_FATAL_PARAM_ERROR(!released_.exchange(true), "Temp file is already released");
    return file_;
}
...
const std::filesystem::path &DependentTmpFileImpl::release() {
    bool already_released = released_.exchange(true);
    CHECK_FATAL_PARAM_ERROR(!already_released, "Temp file is already released");
    return file_;
}
