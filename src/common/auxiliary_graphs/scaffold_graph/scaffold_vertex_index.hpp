...
public:
    ...
    const VertexEntryT &GetHeadEntry(const ScaffoldVertex &vertex) const { return vertex_to_entry_.at(vertex); }
    ...
    const_iterator GetHeadBegin(const ScaffoldVertex &vertex) const { return vertex_to_entry_.at(vertex).begin(); }
    const_iterator GetHeadEnd(const ScaffoldVertex &vertex) const { return vertex_to_entry_.at(vertex).end(); }
...
private:
    void InsertEntry(const ScaffoldVertex &vertex, VertexEntryT &&entry) { vertex_to_entry_.insert({vertex, entry}); }
...
public:
     ...
     */
    virtual size_t GetTripleIntersectionSize(const ScaffoldVertex &first, const ScaffoldVertex &second,
...
public:
    ...
    size_t GetTripleIntersectionSize(const ScaffoldVertex &first, const ScaffoldVertex &second,
                                     const ScaffoldVertex &third) const override {
        return GetIntersectionSize(third, GetIntersection(first, second));
    }

    SimpleVertexEntry GetIntersection(const SimpleVertexEntry &first, const SimpleVertexEntry &second) const override {
    ...
    SimpleVertexEntry GetTailEntry(const ScaffoldVertex &vertex) override { return GetHeadEntry(vertex); }
...
public:
    ...
    size_t GetIntersectionSize(const ScaffoldVertex &middle, const SimpleVertexEntry &entry) const override {
        ...
        SimpleVertexEntry intersection;
        std::set_intersection(entry.begin(), entry.end(), middle_begin, middle_end,
    ...
    size_t GetHeadSize(const ScaffoldVertex &vertex) const override { return (index_->GetHeadEntry(vertex)).size(); }
    size_t GetTailSize(const ScaffoldVertex &vertex) const override { return (index_->GetTailEntry(vertex)).size(); }
    SimpleVertexEntry GetIntersection(const SimpleVertexEntry &first, const SimpleVertexEntry &second) const override {
        SimpleVertexEntry result;
        std::set_intersection(first.begin(), first.end(), second.begin(), second.end(),
                              std::inserter(result, result.end()));
    ...
    SimpleVertexEntry GetHeadEntry(const ScaffoldVertex &vertex) override { return index_->GetHeadEntry(vertex); }
    SimpleVertexEntry GetTailEntry(const ScaffoldVertex &vertex) override { return index_->GetTailEntry(vertex); }
