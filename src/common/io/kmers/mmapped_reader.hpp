...
class MMappedReader {
    ...
    void map() {
        ...
        if (MappedRegion == MAP_FAILED)
            FATAL_IO_ERROR("mmap(2) failed. Reason: " << strerror(errno) << ". Error code: " << errno);
    }
