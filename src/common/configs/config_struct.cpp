...
std::vector<std::string> CheckedNames(const std::vector<std::pair<std::string, mode_t>>& mapping, mode_t total) {
    CHECK_FATAL_PARAM_ERROR(size_t(total) == mapping.size(), "Names for some modes missing")
    std::vector<std::string> answer;
    for (size_t i = 0; i < mapping.size(); ++i) {
        CHECK_FATAL_PARAM_ERROR(size_t(mapping[i].second) == i, "Id/name mapping error");
...
void load(dataset &ds,
    ...
    CHECK_FATAL_FILE_NOT_FOUND_ERROR(exists(reads_filename), "File " << reads_filename << " doesn't exist or can't be read!");
    ...
    CHECK_FATAL_FILE_NOT_FOUND_ERROR(exists(reference_genome_filename), "File " << reference_genome_filename << " doesn't exist or can't be read!");
...
void load_cfg(debruijn_config &cfg, boost::property_tree::ptree const &pt,
    ...
    if (pt.count("prelim_pe")) {
        CHECK_FATAL_PARAM_ERROR(!cfg.prelim_pe_params, "Option prelim_pe can be loaded only once");
