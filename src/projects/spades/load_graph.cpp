...
void LoadGraph::run(graph_pack::GraphPack &gp, const char*) {
    ...
    CHECK_FATAL_FILE_NOT_FOUND_ERROR(exists(path), "File " << path << " doesn't exist or can't be read!");
    ...
    if (k != gp.k())
        FATAL_PARAM_ERROR("GFA used k-mer length (k=" << k << ") must match the command line settings (k=" << gp.k() <<")");
}
