...
inline std::istream &operator>>(std::istream &is, PartialGeneInfo &pgi) {
    ...
    if (strand_symbol == "+")
        ...
        pgi.strand = false;
    else CHECK_FATAL_FORMAT_ERROR(false, "Unsupported strand symbol");
    return is;
}
...
PredictionsFromDescFile(const Graph &g,
    ...
    FATAL_FILE_NOT_FOUND_ERROR(exists(desc_file), "File " << desc_file << " doesn't exist or can't be read!");
...
CDSLengthsFromFile(const std::filesystem::path &fn) {
    ...
    FATAL_FILE_NOT_FOUND_ERROR(exists(fn), "File " << fn << " doesn't exist or can't be read!");
...
static GeneInitSeq PredictionsFromFastaFile(const std::filesystem::path &fasta_fn) {
    CHECK_FATAL_FILE_NOT_FOUND_ERROR(exists(fasta_fn), "File " << fasta_fn << " doesn't exist or can't be read!");
